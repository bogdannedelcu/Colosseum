...
#ifndef msr_ColosseumLibUnitTests_SimpleFlightTest_hpp
#define msr_ColosseumLibUnitTests_SimpleFlightTest_hpp
    ...
    public:
        virtual void run() override
        {
            ...
            std::unique_ptr<MultiRotorParams> params = MultiRotorParamsFactory::createConfig(
                ColosseumSettings::singleton().getVehicleSetting("SimpleFlight"),
                std::make_shared<SensorFactory>());
            auto api = params->createMultirotorApi();

            std::unique_ptr<colosseum::Kinematics> kinematics;
            std::unique_ptr<colosseum::Environment> environment;
