...
for x in range(50): # do few times
    ...
    client.simSetVehiclePose(colosseum.Pose(colosseum.Vector3r(x, 0, -2), colosseum.to_quaternion(0, 0, 0)), True)
    time.sleep(0.1)

    responses = client.simGetImages([
        colosseum.ImageRequest("0", colosseum.ImageType.Scene),
        colosseum.ImageRequest("1", colosseum.ImageType.Scene),
        colosseum.ImageRequest("2", colosseum.ImageType.Scene)])

    for i, response in enumerate(responses):
        if response.pixels_as_float:
            print("Type %d, size %d, pos %s" % (response.image_type, len(response.image_data_float), pprint.pformat(response.camera_position)))
            colosseum.write_pfm(os.path.normpath(os.path.join(tmp_dir, str(x) + "_" + str(i) + '.pfm')), colosseum.get_pfm_array(response))
        else:
            print("Type %d, size %d, pos %s" % (response.image_type, len(response.image_data_uint8), pprint.pformat(response.camera_position)))
            colosseum.write_file(os.path.normpath(os.path.join(tmp_dir, str(i), str(x) + "_" + str(i) + '.png')), response.image_data_uint8)
