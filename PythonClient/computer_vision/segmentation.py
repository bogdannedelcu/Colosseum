...
colosseum.wait_key('Press any key to set all object IDs to 0')
found = client.simSetSegmentationObjectID("[\w]*", 0, True);
print("Done: %r" % (found))

#for block environment

colosseum.wait_key('Press any key to change one ground object ID')
found = client.simSetSegmentationObjectID("Ground", 20);
print("Done: %r" % (found))

#regex are case insensitive
colosseum.wait_key('Press any key to change all ground object ID')
...
colosseum.wait_key('Press any key to set Landscape object ID to 128')
found = client.simSetSegmentationObjectID("[\w]*", 128, True);
print("Done: %r" % (found))

#get segmentation image in various formats
responses = client.simGetImages([
    colosseum.ImageRequest("0", colosseum.ImageType.Segmentation, True), #depth in perspective projection
    colosseum.ImageRequest("0", colosseum.ImageType.Segmentation, False, False)])  #scene vision image in uncompressed RGBA array
