...
constexpr char ColosseumROSWrapper::CAM_YML_NAME[];
constexpr char ColosseumROSWrapper::WIDTH_YML_NAME[];
constexpr char ColosseumROSWrapper::HEIGHT_YML_NAME[];
constexpr char ColosseumROSWrapper::K_YML_NAME[];
constexpr char ColosseumROSWrapper::D_YML_NAME[];
constexpr char ColosseumROSWrapper::R_YML_NAME[];
constexpr char ColosseumROSWrapper::P_YML_NAME[];
constexpr char ColosseumROSWrapper::DMODEL_YML_NAME[];

const std::unordered_map<int, std::string> ColosseumROSWrapper::image_type_int_to_string_map_ = {
    { 0, "Scene" },
    { 1, "DepthPlanar" },
    { 2, "DepthPerspective" },
    { 3, "DepthVis" },
...
ColosseumROSWrapper::ColosseumROSWrapper(const std::shared_ptr<rclcpp::Node> nh, const std::shared_ptr<rclcpp::Node> nh_img, const std::shared_ptr<rclcpp::Node> nh_lidar, const std::string& host_ip)
    : is_used_lidar_timer_cb_queue_(false)
    , is_used_img_timer_cb_queue_(false)
    , colosseum_settings_parser_(host_ip)
    , host_ip_(host_ip)
    , colosseum_client_(nullptr)
    , colosseum_client_images_(host_ip)
    , colosseum_client_lidar_(host_ip)
    , nh_(nh)
    , nh_img_(nh_img)
    , nh_lidar_(nh_lidar)
    , isENU_(false)
    , publish_clock_(false)
{
    ...
    if (ColosseumSettings::singleton().simmode_name != ColosseumSettings::kSimModeTypeCar) {
        colosseum_mode_ = COLOSSEUM_MODE::DRONE;
        RCLCPP_INFO(nh_->get_logger(), "Setting ROS wrapper to DRONE mode");
    }
    else {
        colosseum_mode_ = COLOSSEUM_MODE::CAR;
    ...
    RCLCPP_INFO(nh_->get_logger(), "ColosseumROSWrapper Initialized!");
}

void ColosseumROSWrapper::initialize_colosseum()
{
    // todo do not reset if already in air?
    try {

        if (colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
            colosseum_client_ = std::unique_ptr<colosseum::RpcLibClientBase>(new colosseum::MultirotorRpcLibClient(host_ip_));
        }
        else {
            colosseum_client_ = std::unique_ptr<colosseum::RpcLibClientBase>(new colosseum::CarRpcLibClient(host_ip_));
        }
        colosseum_client_->confirmConnection();
        colosseum_client_images_.confirmConnection();
        colosseum_client_lidar_.confirmConnection();

        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            colosseum_client_->enableApiControl(true, vehicle_name_ptr_pair.first); // todo expose as rosservice?
            colosseum_client_->armDisarm(true, vehicle_name_ptr_pair.first); // todo exposes as rosservice?
        }

        origin_geo_point_ = get_origin_geo_point();
        // todo there's only one global origin geopoint for environment. but colosseum API accept a parameter vehicle_name? inside carsimpawnapi.cpp, there's a geopoint being assigned in the constructor. by?
        origin_geo_point_msg_ = get_gps_msg_from_colosseum_geo_point(origin_geo_point_);
    }
...
void ColosseumROSWrapper::initialize_ros()
{

    // ros params
    double update_colosseum_control_every_n_sec;
    nh_->get_parameter("is_vulkan", is_vulkan_);
    nh_->get_parameter("update_colosseum_control_every_n_sec", update_colosseum_control_every_n_sec);
    nh_->get_parameter("publish_clock", publish_clock_);
    nh_->get_parameter_or("world_frame_id", world_frame_id_, world_frame_id_);
    odom_frame_id_ = world_frame_id_ == COLOSSEUM_FRAME_ID ? COLOSSEUM_ODOM_FRAME_ID : ENU_ODOM_FRAME_ID;
    ...
    colosseum_control_callback_group_ = nh_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
    colosseum_control_update_timer_ = nh_->create_wall_timer(std::chrono::duration<double>(update_colosseum_control_every_n_sec), std::bind(&ColosseumROSWrapper::drone_state_timer_cb, this), colosseum_control_callback_group_);
}

void ColosseumROSWrapper::create_ros_pubs_from_settings_json()
{
    // subscribe to control commands on global nodehandle
    gimbal_angle_quat_cmd_sub_ = nh_->create_subscription<colosseum_interfaces::msg::GimbalAngleQuatCmd>("~/gimbal_angle_quat_cmd", 50, std::bind(&ColosseumROSWrapper::gimbal_angle_quat_cmd_cb, this, _1));
    gimbal_angle_euler_cmd_sub_ = nh_->create_subscription<colosseum_interfaces::msg::GimbalAngleEulerCmd>("~/gimbal_angle_euler_cmd", 50, std::bind(&ColosseumROSWrapper::gimbal_angle_euler_cmd_cb, this, _1));
    origin_geo_point_pub_ = nh_->create_publisher<colosseum_interfaces::msg::GPSYaw>("~/origin_geo_point", 10);

    colosseum_img_request_vehicle_name_pair_vec_.clear();
    ...
    for (const auto& curr_vehicle_elem : ColosseumSettings::singleton().vehicles) {
        ...
        vehicle_ros->env_pub_ = nh_->create_publisher<colosseum_interfaces::msg::Environment>(topic_prefix + "/environment", 10);
        ...
        if (colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
            ...
            std::function<void(const colosseum_interfaces::msg::VelCmd::SharedPtr)> fcn_vel_cmd_body_frame_sub = std::bind(&ColosseumROSWrapper::vel_cmd_body_frame_cb, this, _1, vehicle_ros->vehicle_name_);
            drone->vel_cmd_body_frame_sub_ = nh_->create_subscription<colosseum_interfaces::msg::VelCmd>(topic_prefix + "/vel_cmd_body_frame", 1, fcn_vel_cmd_body_frame_sub); // todo ros::TransportHints().tcpNoDelay();

            std::function<void(const colosseum_interfaces::msg::VelCmd::SharedPtr)> fcn_vel_cmd_world_frame_sub = std::bind(&ColosseumROSWrapper::vel_cmd_world_frame_cb, this, _1, vehicle_ros->vehicle_name_);
            drone->vel_cmd_world_frame_sub_ = nh_->create_subscription<colosseum_interfaces::msg::VelCmd>(topic_prefix + "/vel_cmd_world_frame", 1, fcn_vel_cmd_world_frame_sub);

            std::function<bool(std::shared_ptr<colosseum_interfaces::srv::Takeoff::Request>, std::shared_ptr<colosseum_interfaces::srv::Takeoff::Response>)> fcn_takeoff_srvr = std::bind(&ColosseumROSWrapper::takeoff_srv_cb, this, _1, _2, vehicle_ros->vehicle_name_);
            drone->takeoff_srvr_ = nh_->create_service<colosseum_interfaces::srv::Takeoff>(topic_prefix + "/takeoff", fcn_takeoff_srvr);

            std::function<bool(std::shared_ptr<colosseum_interfaces::srv::Land::Request>, std::shared_ptr<colosseum_interfaces::srv::Land::Response>)> fcn_land_srvr = std::bind(&ColosseumROSWrapper::land_srv_cb, this, _1, _2, vehicle_ros->vehicle_name_);
            drone->land_srvr_ = nh_->create_service<colosseum_interfaces::srv::Land>(topic_prefix + "/land", fcn_land_srvr);

            // vehicle_ros.reset_srvr = nh_->create_service(curr_vehicle_name + "/reset",&ColosseumROSWrapper::reset_srv_cb, this);
        }
        else {
            auto car = static_cast<CarROS*>(vehicle_ros.get());
            std::function<void(const colosseum_interfaces::msg::CarControls::SharedPtr)> fcn_car_cmd_sub = std::bind(&ColosseumROSWrapper::car_cmd_cb, this, _1, vehicle_ros->vehicle_name_);
            car->car_cmd_sub_ = nh_->create_subscription<colosseum_interfaces::msg::CarControls>(topic_prefix + "/car_cmd", 1, fcn_car_cmd_sub);
            car->car_state_pub_ = nh_->create_publisher<colosseum_interfaces::msg::CarState>(topic_prefix + "/car_state", 10);
        }
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings) {
                ...
                // todo why does ColosseumSettings::loadCaptureSettings calls ColosseumSettings::initializeCaptureSettings()
                // which initializes default capture settings for _all_ NINE colosseum::ImageCaptureBase::ImageType
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    ImageType curr_image_type = colosseum::Utils::toEnum<ImageType>(capture_setting.image_type);
            ...
            colosseum_img_request_vehicle_name_pair_vec_.push_back(std::make_pair(current_image_request_vec, curr_vehicle_name));
        }
        ...
        for (auto& curr_sensor_map : vehicle_setting->sensors) {
            ...
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Barometer: {
                    SensorPublisher<colosseum_interfaces::msg::Altimeter> sensor_publisher =
                        create_sensor_publisher<colosseum_interfaces::msg::Altimeter>("Barometer", sensor_setting->sensor_name, sensor_setting->sensor_type, curr_vehicle_name + "/altimeter/" + sensor_name, 10);
                ...
                case SensorBase::SensorType::Lidar: {
                    ...
                    colosseum::LidarSimpleParams params;
    ...
    if (vehicle_name_ptr_map_.size() > 1 && colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
        takeoff_all_srvr_ = nh_->create_service<colosseum_interfaces::srv::Takeoff>("~/all_robots/takeoff", std::bind(&ColosseumROSWrapper::takeoff_all_srv_cb, this, _1, _2));
        land_all_srvr_ = nh_->create_service<colosseum_interfaces::srv::Land>("~/all_robots/land", std::bind(&ColosseumROSWrapper::land_all_srv_cb, this, _1, _2));

        vel_cmd_all_body_frame_sub_ = nh_->create_subscription<colosseum_interfaces::msg::VelCmd>("~/all_robots/vel_cmd_body_frame", 1, std::bind(&ColosseumROSWrapper::vel_cmd_all_body_frame_cb, this, _1));
        vel_cmd_all_world_frame_sub_ = nh_->create_subscription<colosseum_interfaces::msg::VelCmd>("~/all_robots/vel_cmd_world_frame", 1, std::bind(&ColosseumROSWrapper::vel_cmd_all_world_frame_cb, this, _1));

        vel_cmd_group_body_frame_sub_ = nh_->create_subscription<colosseum_interfaces::msg::VelCmdGroup>("~/group_of_robots/vel_cmd_body_frame", 1, std::bind(&ColosseumROSWrapper::vel_cmd_group_body_frame_cb, this, _1));
        vel_cmd_group_world_frame_sub_ = nh_->create_subscription<colosseum_interfaces::msg::VelCmdGroup>("~/group_of_robots/vel_cmd_world_frame", 1, std::bind(&ColosseumROSWrapper::vel_cmd_group_world_frame_cb, this, _1));

        takeoff_group_srvr_ = nh_->create_service<colosseum_interfaces::srv::TakeoffGroup>("~/group_of_robots/takeoff", std::bind(&ColosseumROSWrapper::takeoff_group_srv_cb, this, _1, _2));
        land_group_srvr_ = nh_->create_service<colosseum_interfaces::srv::LandGroup>("~/group_of_robots/land", std::bind(&ColosseumROSWrapper::land_group_srv_cb, this, _1, _2));
    }

    // todo add per vehicle reset in ColosseumLib API
    reset_srvr_ = nh_->create_service<colosseum_interfaces::srv::Reset>("~/reset", std::bind(&ColosseumROSWrapper::reset_srv_cb, this, _1, _2));
    ...
    if (!colosseum_img_request_vehicle_name_pair_vec_.empty()) {
        double update_colosseum_img_response_every_n_sec;
        nh_->get_parameter("update_colosseum_img_response_every_n_sec", update_colosseum_img_response_every_n_sec);
        auto cb = nh_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
        colosseum_img_callback_groups_.push_back(cb);
        colosseum_img_response_timer_ = nh_img_->create_wall_timer(std::chrono::duration<double>(update_colosseum_img_response_every_n_sec), std::bind(&ColosseumROSWrapper::img_response_timer_cb, this), cb);
    ...
    if (lidar_cnt > 0) {
        ...
        auto cb = nh_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
        colosseum_lidar_callback_groups_.push_back(cb);
        colosseum_lidar_update_timer_ = nh_lidar_->create_wall_timer(std::chrono::duration<double>(update_lidar_every_n_sec), std::bind(&ColosseumROSWrapper::lidar_timer_cb, this), cb);
...
template <typename T>
const SensorPublisher<T> ColosseumROSWrapper::create_sensor_publisher(const std::string& sensor_type_name, const std::string& sensor_name,
...
bool ColosseumROSWrapper::takeoff_srv_cb(std::shared_ptr<colosseum_interfaces::srv::Takeoff::Request> request, std::shared_ptr<colosseum_interfaces::srv::Takeoff::Response> response, const std::string& vehicle_name)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);

    if (request->wait_on_last_task)
        static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->takeoffAsync(20, vehicle_name)->waitOnLastTask(); // todo value for timeout_sec?
    // response->success =
    else
        static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->takeoffAsync(20, vehicle_name);
    // response->success =

    return true;
}

bool ColosseumROSWrapper::takeoff_group_srv_cb(std::shared_ptr<colosseum_interfaces::srv::TakeoffGroup::Request> request, std::shared_ptr<colosseum_interfaces::srv::TakeoffGroup::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
bool ColosseumROSWrapper::takeoff_all_srv_cb(std::shared_ptr<colosseum_interfaces::srv::Takeoff::Request> request, std::shared_ptr<colosseum_interfaces::srv::Takeoff::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
    ...
    if (request->wait_on_last_task)
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->takeoffAsync(20, vehicle_name_ptr_pair.first)->waitOnLastTask(); // todo value for timeout_sec?
    // response->success =
    else
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->takeoffAsync(20, vehicle_name_ptr_pair.first);
    // response->success =

    return true;
}

bool ColosseumROSWrapper::land_srv_cb(std::shared_ptr<colosseum_interfaces::srv::Land::Request> request, std::shared_ptr<colosseum_interfaces::srv::Land::Response> response, const std::string& vehicle_name)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);

    if (request->wait_on_last_task)
        static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->landAsync(60, vehicle_name)->waitOnLastTask();
    else
        static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->landAsync(60, vehicle_name);

    return true; //todo
}

bool ColosseumROSWrapper::land_group_srv_cb(std::shared_ptr<colosseum_interfaces::srv::LandGroup::Request> request, std::shared_ptr<colosseum_interfaces::srv::LandGroup::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
bool ColosseumROSWrapper::land_all_srv_cb(std::shared_ptr<colosseum_interfaces::srv::Land::Request> request, std::shared_ptr<colosseum_interfaces::srv::Land::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
    ...
    if (request->wait_on_last_task)
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->landAsync(60, vehicle_name_ptr_pair.first)->waitOnLastTask();
    else
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->landAsync(60, vehicle_name_ptr_pair.first);

    return true; //todo
}

// todo add reset by vehicle_name API to colosseum
// todo not async remove wait_on_last_task
bool ColosseumROSWrapper::reset_srv_cb(std::shared_ptr<colosseum_interfaces::srv::Reset::Request> request, std::shared_ptr<colosseum_interfaces::srv::Reset::Response> response)
{
    unused(request);
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);

    colosseum_client_->reset();
    return true; //todo
}

tf2::Quaternion ColosseumROSWrapper::get_tf2_quat(const colosseum::Quaternionr& colosseum_quat) const
{
    return tf2::Quaternion(colosseum_quat.x(), colosseum_quat.y(), colosseum_quat.z(), colosseum_quat.w());
}

colosseum::Quaternionr ColosseumROSWrapper::get_colosseum_quat(const geometry_msgs::msg::Quaternion& geometry_msgs_quat) const
{
    return colosseum::Quaternionr(geometry_msgs_quat.w, geometry_msgs_quat.x, geometry_msgs_quat.y, geometry_msgs_quat.z);
}

colosseum::Quaternionr ColosseumROSWrapper::get_colosseum_quat(const tf2::Quaternion& tf2_quat) const
{
    return colosseum::Quaternionr(tf2_quat.w(), tf2_quat.x(), tf2_quat.y(), tf2_quat.z());
}

void ColosseumROSWrapper::car_cmd_cb(const colosseum_interfaces::msg::CarControls::SharedPtr msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
colosseum::Pose ColosseumROSWrapper::get_colosseum_pose(const float& x, const float& y, const float& z, const colosseum::Quaternionr& colosseum_quat) const
{
    return colosseum::Pose(colosseum::Vector3r(x, y, z), colosseum_quat);
}

void ColosseumROSWrapper::vel_cmd_body_frame_cb(const colosseum_interfaces::msg::VelCmd::SharedPtr msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(control_mutex_);

    auto drone = static_cast<MultiRotorROS*>(vehicle_name_ptr_map_[vehicle_name].get());
    drone->vel_cmd_ = get_colosseum_body_vel_cmd(*msg, drone->curr_drone_state_.kinematics_estimated.pose.orientation);
    drone->has_vel_cmd_ = true;
}

void ColosseumROSWrapper::vel_cmd_group_body_frame_cb(const colosseum_interfaces::msg::VelCmdGroup::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);

    for (const auto& vehicle_name : msg->vehicle_names) {
        auto drone = static_cast<MultiRotorROS*>(vehicle_name_ptr_map_[vehicle_name].get());
        drone->vel_cmd_ = get_colosseum_body_vel_cmd(msg->vel_cmd, drone->curr_drone_state_.kinematics_estimated.pose.orientation);
        drone->has_vel_cmd_ = true;
    }
}

void ColosseumROSWrapper::vel_cmd_all_body_frame_cb(const colosseum_interfaces::msg::VelCmd::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void ColosseumROSWrapper::vel_cmd_world_frame_cb(const colosseum_interfaces::msg::VelCmd::SharedPtr msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(control_mutex_);

    auto drone = static_cast<MultiRotorROS*>(vehicle_name_ptr_map_[vehicle_name].get());
    drone->vel_cmd_ = get_colosseum_world_vel_cmd(*msg);
    drone->has_vel_cmd_ = true;
}

// this is kinda unnecessary but maybe it makes life easier for the end user.
void ColosseumROSWrapper::vel_cmd_group_world_frame_cb(const colosseum_interfaces::msg::VelCmdGroup::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);

    for (const auto& vehicle_name : msg->vehicle_names) {
        auto drone = static_cast<MultiRotorROS*>(vehicle_name_ptr_map_[vehicle_name].get());
        drone->vel_cmd_ = get_colosseum_world_vel_cmd(msg->vel_cmd);
        drone->has_vel_cmd_ = true;
    }
}

void ColosseumROSWrapper::vel_cmd_all_world_frame_cb(const colosseum_interfaces::msg::VelCmd::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void ColosseumROSWrapper::gimbal_angle_quat_cmd_cb(const colosseum_interfaces::msg::GimbalAngleQuatCmd::SharedPtr gimbal_angle_quat_cmd_msg)
{
    ...
    try {
        ...
        gimbal_cmd_.target_quat = get_colosseum_quat(quat_control_cmd); // colosseum uses wxyz
...
// 3. call colosseum client's setCameraPose which sets camera pose wrt world (or takeoff?) ned frame. todo
void ColosseumROSWrapper::gimbal_angle_euler_cmd_cb(const colosseum_interfaces::msg::GimbalAngleEulerCmd::SharedPtr gimbal_angle_euler_cmd_msg)
{
    try {
        tf2::Quaternion quat_control_cmd;
        ...
        quat_control_cmd.normalize();
        gimbal_cmd_.target_quat = get_colosseum_quat(quat_control_cmd);
...
colosseum_interfaces::msg::CarState ColosseumROSWrapper::get_roscarstate_msg_from_car_state(const colosseum::CarApiBase::CarState& car_state) const
{
    colosseum_interfaces::msg::CarState state_msg;
...
nav_msgs::msg::Odometry ColosseumROSWrapper::get_odom_msg_from_kinematic_state(const colosseum::Kinematics::State& kinematics_estimated) const
{
...
nav_msgs::msg::Odometry ColosseumROSWrapper::get_odom_msg_from_car_state(const colosseum::CarApiBase::CarState& car_state) const
{
    return get_odom_msg_from_kinematic_state(car_state.kinematics_estimated);
}

nav_msgs::msg::Odometry ColosseumROSWrapper::get_odom_msg_from_multirotor_state(const colosseum::MultirotorState& drone_state) const
{
...
sensor_msgs::msg::PointCloud2 ColosseumROSWrapper::get_lidar_msg_from_colosseum(const colosseum::LidarData& lidar_data, const std::string& vehicle_name, const std::string& sensor_name) const
{
    ...
    if (lidar_data.point_cloud.size() > 3) {
        ...
        if (isENU_) {
            try {
                ...
                auto transformStampedENU = tf_buffer_->lookupTransform(COLOSSEUM_FRAME_ID, vehicle_name, rclcpp::Time(0), rclcpp::Duration::from_nanoseconds(1));
...
colosseum_interfaces::msg::Environment ColosseumROSWrapper::get_environment_msg_from_colosseum(const colosseum::Environment::State& env_data) const
{
    colosseum_interfaces::msg::Environment env_msg;
...
sensor_msgs::msg::MagneticField ColosseumROSWrapper::get_mag_msg_from_colosseum(const colosseum::MagnetometerBase::Output& mag_data) const
{
...
// todo covariances
sensor_msgs::msg::NavSatFix ColosseumROSWrapper::get_gps_msg_from_colosseum(const colosseum::GpsBase::Output& gps_data) const
{
...
sensor_msgs::msg::Range ColosseumROSWrapper::get_range_from_colosseum(const colosseum::DistanceSensorData& dist_data) const
{
...
colosseum_interfaces::msg::Altimeter ColosseumROSWrapper::get_altimeter_msg_from_colosseum(const colosseum::BarometerBase::Output& alt_data) const
{
    colosseum_interfaces::msg::Altimeter alt_msg;
...
// todo covariances
sensor_msgs::msg::Imu ColosseumROSWrapper::get_imu_msg_from_colosseum(const colosseum::ImuBase::Output& imu_data) const
{
    sensor_msgs::msg::Imu imu_msg;
    // imu_msg.header.frame_id = "/colosseum/odom_local_ned";// todo multiple drones
...
void ColosseumROSWrapper::publish_odom_tf(const nav_msgs::msg::Odometry& odom_msg)
{
...
colosseum_interfaces::msg::GPSYaw ColosseumROSWrapper::get_gps_msg_from_colosseum_geo_point(const colosseum::GeoPoint& geo_point) const
{
    colosseum_interfaces::msg::GPSYaw gps_msg;
...
sensor_msgs::msg::NavSatFix ColosseumROSWrapper::get_gps_sensor_msg_from_colosseum_geo_point(const colosseum::GeoPoint& geo_point) const
{
    sensor_msgs::msg::NavSatFix gps_msg;
    gps_msg.latitude = geo_point.latitude;
    gps_msg.longitude = geo_point.longitude;
...
colosseum::GeoPoint ColosseumROSWrapper::get_origin_geo_point() const
{
    colosseum::HomeGeoPoint geo_point = ColosseumSettings::singleton().origin_geopoint;
    return geo_point.home_geo_point;
}

VelCmd ColosseumROSWrapper::get_colosseum_world_vel_cmd(const colosseum_interfaces::msg::VelCmd& msg) const
{
    VelCmd vel_cmd;
    vel_cmd.x = msg.twist.linear.x;
    vel_cmd.y = msg.twist.linear.y;
    vel_cmd.z = msg.twist.linear.z;
    vel_cmd.drivetrain = colosseum::DrivetrainType::MaxDegreeOfFreedom;
    vel_cmd.yaw_mode.is_rate = true;
    vel_cmd.yaw_mode.yaw_or_rate = math_common::rad2deg(msg.twist.angular.z);
    return vel_cmd;
}

VelCmd ColosseumROSWrapper::get_colosseum_body_vel_cmd(const colosseum_interfaces::msg::VelCmd& msg, const colosseum::Quaternionr& colosseum_quat) const
{
    VelCmd vel_cmd;
    double roll, pitch, yaw;
    tf2::Matrix3x3(get_tf2_quat(colosseum_quat)).getRPY(roll, pitch, yaw); // ros uses xyzw
    ...
    vel_cmd.drivetrain = colosseum::DrivetrainType::MaxDegreeOfFreedom;
    vel_cmd.yaw_mode.is_rate = true;
    // colosseum uses degrees
    vel_cmd.yaw_mode.yaw_or_rate = math_common::rad2deg(msg.twist.angular.z);

    return vel_cmd;
}

geometry_msgs::msg::Transform ColosseumROSWrapper::get_transform_msg_from_colosseum(const colosseum::Vector3r& position, const colosseum::ColosseumSettings::Rotation& rotation)
{
...
geometry_msgs::msg::Transform ColosseumROSWrapper::get_transform_msg_from_colosseum(const colosseum::Vector3r& position, const colosseum::Quaternionr& quaternion)
{
    geometry_msgs::msg::Transform transform;
    transform.translation.x = position.x();
    transform.translation.y = position.y();
...
void ColosseumROSWrapper::drone_state_timer_cb()
{
    try {
        ...
        if (!colosseum_client_->simIsPaused()) {
            // colosseum_client needs to provide the simulation time in a future version of the API
...
void ColosseumROSWrapper::update_and_publish_static_transforms(VehicleROS* vehicle_ros)
{
...
rclcpp::Time ColosseumROSWrapper::update_state()
{
    ...
    //colosseum::Environment::State env = colosseum_client_->simGetGroundTruthEnvironment("");
    //curr_ros_time = rclcpp::Time(env.clock().nowNanos());

    // iterate over drones
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        rclcpp::Time vehicle_time;
        // get drone state from colosseum
        auto& vehicle_ros = vehicle_name_ptr_pair.second;

        // vehicle environment, we can get ambient temperature here and other truths
        auto env_data = colosseum_client_->simGetGroundTruthEnvironment(vehicle_ros->vehicle_name_);

        if (colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
            auto drone = static_cast<MultiRotorROS*>(vehicle_ros.get());
            auto rpc = static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get());
            ...
            vehicle_ros->gps_sensor_msg_ = get_gps_sensor_msg_from_colosseum_geo_point(drone->curr_drone_state_.gps_location);
            ...
            auto car = static_cast<CarROS*>(vehicle_ros.get());
            auto rpc = static_cast<colosseum::CarRpcLibClient*>(colosseum_client_.get());
            ...
            vehicle_ros->gps_sensor_msg_ = get_gps_sensor_msg_from_colosseum_geo_point(env_data.geo_point);
            vehicle_ros->gps_sensor_msg_.header.stamp = vehicle_time;

            vehicle_ros->curr_odom_ = get_odom_msg_from_car_state(car->curr_car_state_);

            colosseum_interfaces::msg::CarState state_msg = get_roscarstate_msg_from_car_state(car->curr_car_state_);
        ...
        colosseum_interfaces::msg::Environment env_msg = get_environment_msg_from_colosseum(env_data);
        env_msg.header.frame_id = vehicle_ros->vehicle_name_;
        env_msg.header.stamp = vehicle_time;
        vehicle_ros->env_msg_ = env_msg;

        // convert colosseum drone state to ROS msgs
...
void ColosseumROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        auto& vehicle_ros = vehicle_name_ptr_pair.second;
        ...
        if (colosseum_mode_ == COLOSSEUM_MODE::CAR) {
        ...
        for (auto& sensor_publisher : vehicle_ros->barometer_pubs_) {
            auto baro_data = colosseum_client_->getBarometerData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name_);
            colosseum_interfaces::msg::Altimeter alt_msg = get_altimeter_msg_from_colosseum(baro_data);
        ...
        for (auto& sensor_publisher : vehicle_ros->imu_pubs_) {
            auto imu_data = colosseum_client_->getImuData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name_);
            sensor_msgs::msg::Imu imu_msg = get_imu_msg_from_colosseum(imu_data);
        ...
        for (auto& sensor_publisher : vehicle_ros->distance_pubs_) {
            auto distance_data = colosseum_client_->getDistanceSensorData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name_);
            sensor_msgs::msg::Range dist_msg = get_range_from_colosseum(distance_data);
        ...
        for (auto& sensor_publisher : vehicle_ros->gps_pubs_) {
            auto gps_data = colosseum_client_->getGpsData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name_);
            sensor_msgs::msg::NavSatFix gps_msg = get_gps_msg_from_colosseum(gps_data);
        ...
        for (auto& sensor_publisher : vehicle_ros->magnetometer_pubs_) {
            auto mag_data = colosseum_client_->getMagnetometerData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name_);
            sensor_msgs::msg::MagneticField mag_msg = get_mag_msg_from_colosseum(mag_data);
...
void ColosseumROSWrapper::update_commands()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        auto& vehicle_ros = vehicle_name_ptr_pair.second;
        ...
        if (colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
            ...
            // send control commands from the last callback to colosseum
            if (drone->has_vel_cmd_) {
                std::lock_guard<std::mutex> guard(control_mutex_);
                static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get())->moveByVelocityAsync(drone->vel_cmd_.x, drone->vel_cmd_.y, drone->vel_cmd_.z, vel_cmd_duration_, colosseum::DrivetrainType::MaxDegreeOfFreedom, drone->vel_cmd_.yaw_mode, drone->vehicle_name_);
            }
            ...
            if (car->has_car_cmd_) {
                std::lock_guard<std::mutex> guard(control_mutex_);
                static_cast<colosseum::CarRpcLibClient*>(colosseum_client_.get())->setCarControls(car->car_cmd_, vehicle_ros->vehicle_name_);
            }
    ...
    if (has_gimbal_cmd_) {
        std::lock_guard<std::mutex> guard(control_mutex_);
        colosseum_client_->simSetCameraPose(gimbal_cmd_.camera_name, get_colosseum_pose(0, 0, 0, gimbal_cmd_.target_quat), gimbal_cmd_.vehicle_name);
    }

    has_gimbal_cmd_ = false;
}

// colosseum uses nans for zeros in settings.json. we set them to zeros here for handling tfs in ROS
void ColosseumROSWrapper::set_nans_to_zeros_in_pose(VehicleSetting& vehicle_setting) const
{
...
void ColosseumROSWrapper::set_nans_to_zeros_in_pose(const VehicleSetting& vehicle_setting, CameraSetting& camera_setting) const
{
...
void ColosseumROSWrapper::convert_tf_msg_to_enu(geometry_msgs::msg::TransformStamped& tf_msg)
{
...
geometry_msgs::msg::Transform ColosseumROSWrapper::get_camera_optical_tf_from_body_tf(const geometry_msgs::msg::Transform& body_tf) const
{
    geometry_msgs::msg::Transform optical_tf = body_tf; //same translation
    auto opticalQ = colosseum::Quaternionr(optical_tf.rotation.w, optical_tf.rotation.x, optical_tf.rotation.y, optical_tf.rotation.z);
    if (isENU_)
        opticalQ *= colosseum::Quaternionr(0.7071068, -0.7071068, 0, 0); //CamOptical in CamBodyENU is rmat[1,0,0;0,0,-1;0,1,0]==xyzw[-0.7071068,0,0,0.7071068]
    else
        opticalQ *= colosseum::Quaternionr(0.5, 0.5, 0.5, 0.5); //CamOptical in CamBodyNED is rmat[0,0,1;1,0,0;0,1,0]==xyzw[0.5,0.5,0.5,0.5]
...
void ColosseumROSWrapper::append_static_vehicle_tf(VehicleROS* vehicle_ros, const VehicleSetting& vehicle_setting)
{
    ...
    vehicle_tf_msg.transform = get_transform_msg_from_colosseum(vehicle_setting.position, vehicle_setting.rotation);
...
void ColosseumROSWrapper::append_static_lidar_tf(VehicleROS* vehicle_ros, const std::string& lidar_name, const colosseum::LidarSimpleParams& lidar_setting)
{
    ...
    lidar_tf_msg.transform = get_transform_msg_from_colosseum(lidar_setting.relative_pose.position, lidar_setting.relative_pose.orientation);
...
void ColosseumROSWrapper::append_static_camera_tf(VehicleROS* vehicle_ros, const std::string& camera_name, const CameraSetting& camera_setting)
{
    ...
    static_cam_tf_body_msg.transform = get_transform_msg_from_colosseum(camera_setting.position, camera_setting.rotation);
...
void ColosseumROSWrapper::img_response_timer_cb()
{
    try {
        int image_response_idx = 0;
        for (const auto& colosseum_img_request_vehicle_name_pair : colosseum_img_request_vehicle_name_pair_vec_) {
            const std::vector<ImageResponse>& img_response = colosseum_client_images_.simGetImages(colosseum_img_request_vehicle_name_pair.first, colosseum_img_request_vehicle_name_pair.second);

            if (img_response.size() == colosseum_img_request_vehicle_name_pair.first.size()) {
                process_and_publish_img_response(img_response, image_response_idx, colosseum_img_request_vehicle_name_pair.second);
...
void ColosseumROSWrapper::lidar_timer_cb()
{
    try {
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->lidar_pubs_.empty()) {
                for (auto& lidar_publisher : vehicle_name_ptr_pair.second->lidar_pubs_) {
                    auto lidar_data = colosseum_client_lidar_.getLidarData(lidar_publisher.sensor_name, vehicle_name_ptr_pair.first);
                    sensor_msgs::msg::PointCloud2 lidar_msg = get_lidar_msg_from_colosseum(lidar_data, vehicle_name_ptr_pair.first, lidar_publisher.sensor_name);
...
std::shared_ptr<sensor_msgs::msg::Image> ColosseumROSWrapper::get_img_msg_from_response(const ImageResponse& img_response,
...
std::shared_ptr<sensor_msgs::msg::Image> ColosseumROSWrapper::get_depth_img_msg_from_response(const ImageResponse& img_response,
                                                                                           const rclcpp::Time curr_ros_time,
                                                                                           const std::string frame_id)
{
    unused(curr_ros_time);
...
sensor_msgs::msg::CameraInfo ColosseumROSWrapper::generate_cam_info(const std::string& camera_name,
...
void ColosseumROSWrapper::process_and_publish_img_response(const std::vector<ImageResponse>& img_response_vec, const int img_response_idx, const std::string& vehicle_name)
{
    // todo add option to use colosseum time (image_response.TTimePoint) like Gazebo /use_sim_time param
    rclcpp::Time curr_ros_time = nh_->now();
    ...
    for (const auto& curr_img_response : img_response_vec) {
        ...
        // colosseum::CameraInfo camera_info = colosseum_client_.simGetCameraInfo(curr_img_response.camera_name);
...
// camera poses are obtained from colosseum's client API which are in (local) NED frame.
// We first do a change of basis to camera optical frame (Z forward, X right, Y down)
void ColosseumROSWrapper::publish_camera_tf(const ImageResponse& img_response, const rclcpp::Time& ros_time, const std::string& frame_id, const std::string& child_frame_id)
{
    unused(ros_time);
    ...
    cam_tf_body_msg.transform = get_transform_msg_from_colosseum(img_response.camera_position, img_response.camera_orientation);
...
void ColosseumROSWrapper::convert_yaml_to_simple_mat(const YAML::Node& node, SimpleMatrix& m) const
{
    int rows, cols;
...
void ColosseumROSWrapper::read_params_from_yaml_and_fill_cam_info_msg(const std::string& file_name, sensor_msgs::msg::CameraInfo& cam_info) const
{
