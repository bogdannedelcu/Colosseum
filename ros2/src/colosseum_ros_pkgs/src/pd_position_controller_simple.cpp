...
{
    vel_cmd_ = colosseum_interfaces::msg::VelCmd();
    // ROS params
    double update_control_every_n_sec;
    nh_->get_parameter("update_control_every_n_sec", update_control_every_n_sec);

    auto parameters_client = std::make_shared<rclcpp::SyncParametersClient>(nh_, "/colosseum_node");
    ...
    // ROS publishers
    colosseum_vel_cmd_world_frame_pub_ = nh_->create_publisher<colosseum_interfaces::msg::VelCmd>("/colosseum_node/" + vehicle_name + "/vel_cmd_world_frame", 1);

    // ROS subscribers
    colosseum_odom_sub_ = nh_->create_subscription<nav_msgs::msg::Odometry>("/colosseum_node/" + vehicle_name + "/odom_local_ned", 50, std::bind(&PIDPositionController::colosseum_odom_cb, this, _1));
    home_geopoint_sub_ = nh_->create_subscription<colosseum_interfaces::msg::GPSYaw>("/colosseum_node/home_geo_point", 50, std::bind(&PIDPositionController::home_geopoint_cb, this, _1));
    // todo publish this under global nodehandle / "colosseum node" and hide it from user
    local_position_goal_srvr_ = nh_->create_service<colosseum_interfaces::srv::SetLocalPosition>("/colosseum_node/local_position_goal", std::bind(&PIDPositionController::local_position_goal_srv_cb, this, _1, _2));
    local_position_goal_override_srvr_ = nh_->create_service<colosseum_interfaces::srv::SetLocalPosition>("/colosseum_node/local_position_goal/override", std::bind(&PIDPositionController::local_position_goal_srv_override_cb, this, _1, _2));
    gps_goal_srvr_ = nh_->create_service<colosseum_interfaces::srv::SetGPSPosition>("/colosseum_node/gps_goal", std::bind(&PIDPositionController::gps_goal_srv_cb, this, _1, _2));
    gps_goal_override_srvr_ = nh_->create_service<colosseum_interfaces::srv::SetGPSPosition>("/colosseum_node/gps_goal/override", std::bind(&PIDPositionController::gps_goal_srv_override_cb, this, _1, _2));
...
void PIDPositionController::colosseum_odom_cb(const nav_msgs::msg::Odometry::SharedPtr odom_msg)
{
    has_odom_ = true;
    curr_odom_ = *odom_msg;
...
bool PIDPositionController::local_position_goal_srv_cb(const std::shared_ptr<colosseum_interfaces::srv::SetLocalPosition::Request> request, std::shared_ptr<colosseum_interfaces::srv::SetLocalPosition::Response> response)
{
    unused(response);
...
bool PIDPositionController::local_position_goal_srv_override_cb(const std::shared_ptr<colosseum_interfaces::srv::SetLocalPosition::Request> request, std::shared_ptr<colosseum_interfaces::srv::SetLocalPosition::Response> response)
{
    unused(response);
    // this tells the update timer callback to not do active hovering
    if (!got_goal_once_)
...
void PIDPositionController::home_geopoint_cb(const colosseum_interfaces::msg::GPSYaw::SharedPtr gps_msg)
{
    if (has_home_geo_)
        return;
...
bool PIDPositionController::gps_goal_srv_cb(const std::shared_ptr<colosseum_interfaces::srv::SetGPSPosition::Request> request, std::shared_ptr<colosseum_interfaces::srv::SetGPSPosition::Response> response)
{
    if (!has_home_geo_) {
    ...
    if (!has_goal_) {
        colosseum::GeoPoint goal_gps_point(request->latitude, request->longitude, request->altitude);
        colosseum::GeoPoint gps_home(gps_home_msg_.latitude, gps_home_msg_.longitude, gps_home_msg_.altitude);
        ...
        else // use colosseum::GeodeticToNedFast
        {
            RCLCPP_INFO_STREAM(nh_->get_logger(), "home geopoint: lat=" << gps_home.latitude << " long=" << gps_home.longitude << " alt=" << gps_home.altitude << " yaw="
                                                                        << "todo");
            colosseum::Vector3r ned_goal = colosseum::EarthUtils::GeodeticToNedFast(goal_gps_point, gps_home);
...
// todo do relative altitude, or add an option for the same?
bool PIDPositionController::gps_goal_srv_override_cb(const std::shared_ptr<colosseum_interfaces::srv::SetGPSPosition::Request> request, std::shared_ptr<colosseum_interfaces::srv::SetGPSPosition::Response> response)
{
    if (!has_home_geo_) {
        RCLCPP_ERROR_STREAM(nh_->get_logger(), "I don't have home GPS coord. Can't go to GPS goal!");
        response->success = false;
...
{
    colosseum_vel_cmd_world_frame_pub_->publish(vel_cmd_);
}
