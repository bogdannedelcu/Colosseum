...
def generate_launch_description():
    ...
    colosseum_node = Node(
            package='colosseum_ros_pkgs',
            executable='colosseum_node',
            name='colosseum_node',
            output='screen',
            parameters=[{
                'is_vulkan': False,
                'update_colosseum_img_response_every_n_sec': 0.05,
                'update_colosseum_control_every_n_sec': 0.01,
    ...
    static_transforms = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('colosseum_ros_pkgs'), 'launch/static_transforms.launch.py')
        )
    )
