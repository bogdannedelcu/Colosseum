...
namespace colosseum
{
    ...
    public:
        ...
        virtual void gotoGoalSGM(const Pose& goal_pose, RpcLibClientBase& client, const std::vector<ImageCaptureBase::ImageRequest>& request, CStateStereo* p_state)
        {
            ...
            */
                ...
                const Pose next_pose = getNextPose(sgm_depth_image, goal_pose.position, current_pose, params_.control_loop_period);
	...
	*/
        ...
        std::vector<int> spiralOrder(int m, int n, int idx)
        {
            ...
            while (i < cnt) {
                if (0 <= r && r < n && 0 <= c && c < m) {
                    spiral_idx.push_back(current_idx);
                    ++i;
                }
                r += dy[dir];
                c += dx[dir];
                ...
                else if (down == dir && r == rmax) {
                    dir = left;
                    rmax++;
                } //down
                else if (left == dir && c == cmin) {
                    dir = up;
                    cmin--;
                } //left
                else if (up == dir && r == rmin) {
                    dir = right;
                    rmin--;
                } //up
            }
        ...
        Pose rotateToGoal(Pose current_pose, Vector3r goal)
        {
            ...
            if (diffAngle > 0) {
                slerp_alpha = params_.rotation_step_limit / diffAngle;
            }
            if (slerp_alpha > 1) {
                slerp_alpha = 1;
            }
