...
namespace colosseum
{
    ...
    public:
        DepthNavOptAStar(const Params& params = Params())
            : params_(params), sample_rays(params.ray_samples_count + extra_rays), //add two more rays, for origin and goal
            rnd_width_(0, params.env_width - 1)
            , rnd_height_(0, params.env_height - 1)
        {
        ...
        virtual void gotoGoal(const Pose& goal_pose, RpcLibClientBase& client)
        {
            ...
            do {
                ...
                const Pose next_pose = getNextPose(response.at(0).image_data_float, goal_pose.position, current_pose, params_.control_loop_period);
    ...
    protected:
        Pose getNextPose(const std::vector<float>& depth_image, const Vector3r& goal, const Pose& current_pose, real_T dt)
        {
            ...
            if (generate_debug_info_) {
                ...
                writeToBmpFile(bmp, params_.depth_width, params_.depth_height, common_utils::FileSystem::combine(std::string("d:\\temp\\111\\"), Utils::stringf("disparity_ % 06d.bmp", iteration_index_)));
            }
            ...
            Quaternionr next_q = min_cost_ray->d1_v.isZero(params_.d1_zero_epsilon) ? VectorMath::toQuaternion(VectorMath::front(), min_cost_ray->d2_v.normalized()) : VectorMath::toQuaternion(VectorMath::front(), min_cost_ray->d1_v.normalized());
