...
roslaunch colosseum_ros_pkgs colosseum_node.launch;
roslaunch colosseum_ros_pkgs rviz.launch;
```

   **Note**: If you get an error running `roslaunch colosseum_ros_pkgs colosseum_node.launch`, run `catkin clean` and try again
...
- `/colosseum_node/origin_geo_point` [colosseum_ros_pkgs/GPSYaw](https://github.com/CodexLabsLLC/Colosseum/tree/main/ros/src/colosseum_ros_pkgs/msg/GPSYaw.msg)
GPS coordinates corresponding to global NED frame. This is set in the colosseum's [settings.json](https://CodexLabsLLC.github.io/Colosseum/settings/) file under the `OriginGeopoint` key.

- `/colosseum_node/VEHICLE_NAME/global_gps` [sensor_msgs/NavSatFix](https://docs.ros.org/api/sensor_msgs/html/msg/NavSatFix.html)
This the current GPS coordinates of the drone in colosseum.

- `/colosseum_node/VEHICLE_NAME/odom_local_ned` [nav_msgs/Odometry](https://docs.ros.org/api/nav_msgs/html/msg/Odometry.html)
Odometry in NED frame (default name: odom_local_ned, launch name and frame type are configurable) wrt take-off point.

- `/colosseum_node/VEHICLE_NAME/CAMERA_NAME/IMAGE_TYPE/camera_info` [sensor_msgs/CameraInfo](https://docs.ros.org/api/sensor_msgs/html/msg/CameraInfo.html)

- `/colosseum_node/VEHICLE_NAME/CAMERA_NAME/IMAGE_TYPE` [sensor_msgs/Image](https://docs.ros.org/api/sensor_msgs/html/msg/Image.html)
  RGB or float image depending on image type requested in settings.json.

- `/tf` [tf2_msgs/TFMessage](https://docs.ros.org/api/tf2_msgs/html/msg/TFMessage.html)

- `/colosseum_node/VEHICLE_NAME/altimeter/SENSOR_NAME` [colosseum_ros_pkgs/Altimeter](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_ros_pkgs/msg/Altimeter.msg)
This the current altimeter reading for altitude, pressure, and [QNH](https://en.wikipedia.org/wiki/QNH)

- `/colosseum_node/VEHICLE_NAME/imu/SENSOR_NAME` [sensor_msgs::Imu](http://docs.ros.org/api/sensor_msgs/html/msg/Imu.html)
IMU sensor data

- `/colosseum_node/VEHICLE_NAME/magnetometer/SENSOR_NAME` [sensor_msgs::MagneticField](http://docs.ros.org/api/sensor_msgs/html/msg/MagneticField.html)
  Meausrement of magnetic field vector/compass

- `/colosseum_node/VEHICLE_NAME/distance/SENSOR_NAME` [sensor_msgs::Range](http://docs.ros.org/api/sensor_msgs/html/msg/Range.html)
  Meausrement of distance from an active ranger, such as infrared or IR

- `/colosseum_node/VEHICLE_NAME/lidar/SENSOR_NAME` [sensor_msgs::PointCloud2](http://docs.ros.org/api/sensor_msgs/html/msg/PointCloud2.html)
  LIDAR pointcloud

#### Subscribers:

- `/colosseum_node/vel_cmd_body_frame` [colosseum_ros_pkgs/VelCmd](https://github.com/CodexLabsLLC/Colosseum/tree/main/ros/src/colosseum_ros_pkgs/msg/VelCmd.msg)
  Ignore `vehicle_name` field, leave it to blank. We will use `vehicle_name` in future for multiple drones.

- `/colosseum_node/vel_cmd_world_frame` [colosseum_ros_pkgs/VelCmd](https://github.com/CodexLabsLLC/Colosseum/tree/main/ros/src/colosseum_ros_pkgs/msg/VelCmd.msg)
  Ignore `vehicle_name` field, leave it to blank. We will use `vehicle_name` in future for multiple drones.

- `/gimbal_angle_euler_cmd` [colosseum_ros_pkgs/GimbalAngleEulerCmd](https://github.com/CodexLabsLLC/Colosseum/tree/main/ros/src/colosseum_ros_pkgs/msg/GimbalAngleEulerCmd.msg)
  Gimbal set point in euler angles.

- `/gimbal_angle_quat_cmd` [colosseum_ros_pkgs/GimbalAngleQuatCmd](https://github.com/CodexLabsLLC/Colosseum/tree/main/ros/src/colosseum_ros_pkgs/msg/GimbalAngleQuatCmd.msg)
  Gimbal set point in quaternion.

- `/colosseum_node/VEHICLE_NAME/car_cmd` [colosseum_ros_pkgs/CarControls](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_ros_pkgs/msg/CarControls.msg)
Throttle, brake, steering and gear selections for control. Both automatic and manual transmission control possible, see the [`car_joy.py`](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_ros_pkgs/scripts/car_joy) script for use.

#### Services:

- `/colosseum_node/VEHICLE_NAME/land` [colosseum_ros_pkgs/Takeoff](https://docs.ros.org/api/std_srvs/html/srv/Empty.html)

- `/colosseum_node/takeoff` [colosseum_ros_pkgs/Takeoff](https://docs.ros.org/api/std_srvs/html/srv/Empty.html)

- `/colosseum_node/reset` [colosseum_ros_pkgs/Reset](https://docs.ros.org/api/std_srvs/html/srv/Empty.html)
 Resets *all* drones

#### Parameters:

- `/colosseum_node/world_frame_id` [string]
  Set in: `$(colosseum_ros_pkgs)/launch/colosseum_node.launch`
  Default: world_ned
  Set to "world_enu" to switch to ENU frames automatically

- `/colosseum_node/odom_frame_id` [string]
  Set in: `$(colosseum_ros_pkgs)/launch/colosseum_node.launch`
...
- `/colosseum_node/coordinate_system_enu` [boolean]
  Set in: `$(colosseum_ros_pkgs)/launch/colosseum_node.launch`
  Default: false
  If you set world_frame_id to "world_enu", this setting will instead default to true

- `/colosseum_node/update_colosseum_control_every_n_sec` [double]
  Set in: `$(colosseum_ros_pkgs)/launch/colosseum_node.launch`
  Default: 0.01 seconds.
  Timer callback frequency for updating drone odom and state from colosseum, and sending in control commands.
  The current RPClib interface to unreal engine maxes out at 50 Hz.
  Timer callbacks in ROS run at maximum rate possible, so it's best to not touch this parameter.

- `/colosseum_node/update_colosseum_img_response_every_n_sec` [double]
  Set in: `$(colosseum_ros_pkgs)/launch/colosseum_node.launch`
  Default: 0.01 seconds.
  Timer callback frequency for receiving images from all cameras in colosseum.
  The speed will depend on number of images requested and their resolution.
  Timer callbacks in ROS run at maximum rate possible, so it's best to not touch this parameter.

- `/colosseum_node/publish_clock` [double]
  Set in: `$(colosseum_ros_pkgs)/launch/colosseum_node.launch`
  Default: false
...
- `/colosseum_node/VEHICLE_NAME/gps_goal` [Request: [srv/SetGPSPosition](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_ros_pkgs/srv/SetGPSPosition.srv)]
  Target gps position + yaw.
  In **absolute** altitude.

- `/colosseum_node/VEHICLE_NAME/local_position_goal` [Request: [srv/SetLocalPosition](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_ros_pkgs/srv/SetLocalPosition.srv)]
  Target local position + yaw in global NED frame.

#### Subscribers:

- `/colosseum_node/origin_geo_point` [colosseum_ros_pkgs/GPSYaw](https://github.com/CodexLabsLLC/Colosseum/tree/main/ros/src/colosseum_ros_pkgs/msg/GPSYaw.msg)
  Listens to home geo coordinates published by `colosseum_node`.

- `/colosseum_node/VEHICLE_NAME/odom_local_ned` [nav_msgs/Odometry](https://docs.ros.org/api/nav_msgs/html/msg/Odometry.html)
  Listens to odometry published by `colosseum_node`

#### Publishers:

- `/vel_cmd_world_frame` [colosseum_ros_pkgs/VelCmd](https://github.com/CodexLabsLLC/Colosseum/tree/main/ros/src/colosseum_ros_pkgs/msg/VelCmd.msg)
  Sends velocity command to `colosseum_node`
...
2. Congratulations, you now have a working Ubuntu subsystem under Windows, you can now go to [Ubuntu 16 / 18 instructions](#setup) and then [How to run Colosseum on Windows and ROS wrapper on WSL](#how-to-run-colosseum-on-windows-and-ros-wrapper-on-wsl)!
...
##### How to run Colosseum on Windows and ROS wrapper on WSL
...
```shell
source devel/setup.bash
roslaunch colosseum_ros_pkgs colosseum_node.launch output:=screen host:=$WSL_HOST_IP
roslaunch colosseum_ros_pkgs rviz.launch
```

### Using Docker for ROS

A Dockerfile is present in the [`tools`](https://github.com/CodexLabsLLC/Colosseum/tree/main/tools/Dockerfile-ROS) directory. To build the `colosseum-ros` image -

```shell
cd tools
docker build -t colosseum-ros -f Dockerfile-ROS .
```

To run, replace the path of the Colosseum folder below -

```shell
docker run --rm -it --net=host -v <your-Colosseum-folder-path>:/home/testuser/Colosseum colosseum-ros:latest bash
```
