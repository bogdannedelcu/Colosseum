...
See [colosseum_ros_pkgs](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_ros_pkgs/README.md) for the ROS API.


## Setup

Make sure that the [colosseum_ros_pkgs Setup](colosseum_ros_pkgs.md) has been completed and the prerequisites installed.

```shell
$ cd PATH_TO/Colosseum/ros
$ catkin build colosseum_tutorial_pkgs
```
...
```shell
catkin build colosseum_tutorial_pkgs -DCMAKE_C_COMPILER=gcc-8 -DCMAKE_CXX_COMPILER=g++-8
```
...
### Single drone with monocular and depth cameras, and lidar
 - Settings.json - [front_stereo_and_center_mono.json](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_tutorial_pkgs/settings/front_stereo_and_center_mono.json)
 ```shell
 $ source PATH_TO/Colosseum/ros/devel/setup.bash
 $ roscd colosseum_tutorial_pkgs
 ...
 $ source PATH_TO/Colosseum/ros/devel/setup.bash
 $ roslaunch colosseum_tutorial_pkgs front_stereo_and_center_mono.launch
 ```
 The above would start rviz with tf's, registered RGBD cloud using [depth_image_proc](https://wiki.ros.org/depth_image_proc) using the [`depth_to_pointcloud` launch file](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_tutorial_pkgs/launch/front_stereo_and_center_mono/depth_to_pointcloud.launch), and the lidar point cloud. 


### Two drones, with cameras, lidar, IMU each
- Settings.json - [two_drones_camera_lidar_imu.json](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_tutorial_pkgs/settings/two_drones_camera_lidar_imu.json) 
...
- Settings.json - [twenty_five_drones.json](https://github.com/CodexLabsLLC/Colosseum/blob/main/ros/src/colosseum_tutorial_pkgs/settings/twenty_five_drones.json) 
