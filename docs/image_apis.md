...
Please read [general API doc](apis.md) first if you are not familiar with Colosseum APIs.
...
#### Using Colosseum Images with NumPy
...
For a more complete ready to run sample code please see [sample code in ColosseumClient project](https://github.com/CodexLabsLLC/Colosseum/tree/main/PythonClient//multirotor/hello_drone.py) for multirotors or [HelloCar sample](https://github.com/CodexLabsLLC/Colosseum/tree/main/PythonClient//car/hello_car.py). This code also demonstrates simple activities such as saving images in files or using `numpy` to manipulate images.

### C++

For a more complete ready to run sample code please see [sample code in HelloDrone project](https://github.com/CodexLabsLLC/Colosseum/tree/main/HelloDrone//main.cpp) for multirotors or [HelloCar project](https://github.com/CodexLabsLLC/Colosseum/tree/main/HelloCar//main.cpp). 

See also [other example code](https://github.com/CodexLabsLLC/Colosseum/tree/main/Examples/DataCollection/StereoImageGenerator.hpp) that generates specified number of stereo images along with ground truth depth and disparity and saving it to [pfm format](pfm.md).
...
[Here's the Python code example](https://github.com/CodexLabsLLC/Colosseum/tree/main/PythonClient//computer_vision/cv_mode.py) to move camera around and capture images.
...
All Camera APIs take in 3 common parameters apart from the API-specific ones, `camera_name`(str), `vehicle_name`(str) and `external`(bool, to indicate [External Camera](settings.md#external-cameras)). Camera and vehicle name is used to get the specific camera, if `external` is set to `true`, then the vehicle name is ignored. Also see [external_camera.py](https://github.com/CodexLabsLLC/Colosseum/blob/main/PythonClient/computer_vision/external_camera.py) for example usage of these APIs.

### Gimbal
You can set stabilization for pitch, roll or yaw for any camera [using settings](settings.md#gimbal).

Please see [example usage](https://github.com/CodexLabsLLC/Colosseum/tree/main/PythonClient//computer_vision/cv_mode.py).
...
### Segmentation
When you specify `ImageType = Segmentation` in `ImageRequest`, you get an image that gives you ground truth segmentation of the scene. At the startup, Colosseum assigns value 0 to 255 to each mesh available in environment. This value is then mapped to a specific color in [the pallet](https://github.com/CodexLabsLLC/Colosseum/blob/main/Unreal/Plugins/Colosseum/Content/HUDAssets/seg_color_palette.png). The RGB values for each object ID can be found in [this file](seg_rgbs.txt).
...
Notice that typical Unreal environments, like Blocks, usually have many other meshes that comprises of same object, for example, "Ground_2", "Ground_3" and so on. As it is tedious to set object ID for all of these meshes, Colosseum also supports regular expressions. For example, the code below sets all meshes which have names starting with "ground" (ignoring case) to 21 with just one line:
...
```python
responses = client.simGetImages([ImageRequest(0, ColosseumImageType.Segmentation, False, False)])
...
A complete ready-to-run example can be found in [segmentation.py](https://github.com/CodexLabsLLC/Colosseum/tree/main/PythonClient//computer_vision/segmentation.py).
...
## Example Code
A complete example of setting vehicle positions at random locations and orientations and then taking images can be found in [GenerateImageGenerator.hpp](https://github.com/CodexLabsLLC/Colosseum/tree/main/Examples/DataCollection/StereoImageGenerator.hpp). This example generates specified number of stereo images and ground truth disparity image and saving it to [pfm format](pfm.md).
