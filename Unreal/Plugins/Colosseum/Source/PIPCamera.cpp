...
{
    static ConstructorHelpers::FObjectFinder<UMaterial> mat_finder(TEXT("Material'/Colosseum/HUDAssets/CameraSensorNoise.CameraSensorNoise'"));
    if (mat_finder.Succeeded()) {
        noise_material_static_ = mat_finder.Object;
    }
    else
        UColosseumBlueprintLib::LogMessageString("Cannot create noise material for the PIPCamera",
                                           "",
                                           LogDebugLevel::Failure);

    static ConstructorHelpers::FObjectFinder<UMaterial> dist_mat_finder(TEXT("Material'/Colosseum/HUDAssets/CameraDistortion.CameraDistortion'"));
    if (dist_mat_finder.Succeeded()) {
        distortion_material_static_ = dist_mat_finder.Object;
        distortion_param_collection_ = Cast<UMaterialParameterCollection>(StaticLoadObject(UMaterialParameterCollection::StaticClass(), NULL, TEXT("'/Colosseum/HUDAssets/DistortionParams.DistortionParams'")));
    }
    else
        UColosseumBlueprintLib::LogMessageString("Cannot create distortion material for the PIPCamera",
                                           "",
                                           LogDebugLevel::Failure);
...
void APIPCamera::PostInitializeComponents()
{
    ...
    //CinemColosseum
    camera_ = UColosseumBlueprintLib::GetActorComponent<UCineCameraComponent>(this, TEXT("CameraComponent"));
    ...
    captures_[Utils::toNumeric(ImageType::Scene)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("SceneCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::DepthPlanar)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("DepthPlanarCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::DepthPerspective)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("DepthPerspectiveCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::DepthVis)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("DepthVisCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::DisparityNormalized)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("DisparityNormalizedCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::Segmentation)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("SegmentationCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::Infrared)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("InfraredCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::SurfaceNormals)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("NormalsCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::OpticalFlow)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("OpticalFlowCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::OpticalFlowVis)] =
        UColosseumBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("OpticalFlowVisCaptureComponent"));
...
colosseum::ProjectionMatrix APIPCamera::getProjectionMatrix(const APIPCamera::ImageType image_type) const
{
    colosseum::ProjectionMatrix mat;
    ...
    if (capture) {
        ...
        //Takes a vector from NORTH-EAST-DOWN coordinates (Colosseum) to EAST-UP-SOUTH coordinates (Unreal). Leaves W coordinate unchanged.
        ...
        FMatrix projMatTransposeInColosseum = coordinateChangeTranspose * proj_mat_transpose;

        //Copy the result to an colosseum::ProjectionMatrix while taking transpose.
        for (auto row = 0; row < 4; ++row)
            for (auto col = 0; col < 4; ++col)
                mat.matrix[col][row] = projMatTransposeInColosseum.M[row][col];
    }
    else
...
void APIPCamera::showToScreen()
{
    ...
    UColosseumBlueprintLib::LogMessage(TEXT("Camera: "), GetName(), LogDebugLevel::Informational);
}
...
void APIPCamera::setCameraPose(const colosseum::Pose& relative_pose)
{
...
colosseum::CameraInfo APIPCamera::getCameraInfo() const
{
    colosseum::CameraInfo camera_info;
...
void APIPCamera::setupCameraFromSettings(const APIPCamera::CameraSetting& camera_setting, const NedTransform& ned_transform)
{
    ...
    for (int image_type = -1; image_type < image_count; ++image_type) {
        ...
        if (image_type >= 0) { //scene capture components
            ...
            copyCameraSettingsToSceneCapture(camera_, captures_[image_type]); //CinemColosseum
        }
            ...
            copyCameraSettingsToAllSceneCapture(camera_); //CinemColosseum
        }
    }
}
...
colosseum::Pose APIPCamera::getPose() const
{
