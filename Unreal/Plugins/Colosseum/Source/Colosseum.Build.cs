...
public class Colosseum : ModuleRules
{
    ...
    private string ColosseumLibPath
    {
        get { return Path.Combine(ModulePath, "ColosseumLib"); }
    }
    private string ColosseumPluginPath
    {
    ...
    private string ProjectBinariesPath
    {
        ...
        {
            return Path.Combine(
                Directory.GetParent(ColosseumPluginPath).Parent.FullName, "Binaries");
        }
    }
    private string ColosseumPluginDependencyPath
    {
        get { return Path.Combine(ColosseumPluginPath, "Dependencies"); }
    }
    ...
    {
        LoadColosseumDependency(Target, "MavLinkCom", "MavLinkCom");

        switch (mode)
        {
            case CompileMode.HeaderOnlyNoRpc:
                PublicDefinitions.Add("COLOSSEUM_HEADER_ONLY=1");
                PublicDefinitions.Add("COLOSSEUM_NO_RPC=1");
                AddLibDependency("ColosseumLib", Path.Combine(ColosseumLibPath, "lib"), "ColosseumLib", Target, false);
                break;

            case CompileMode.HeaderOnlyWithRpc:
                PublicDefinitions.Add("COLOSSEUM_HEADER_ONLY=1");
                AddLibDependency("ColosseumLib", Path.Combine(ColosseumLibPath, "lib"), "ColosseumLib", Target, false);
                LoadColosseumDependency(Target, "rpclib", "rpc");
                break;
    ...
    public Colosseum(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        PrivateDependencyModuleNames.AddRange(new string[] { "UMG", "Slate", "SlateCore", "RenderCore", "EngineSettings" });
        ...
        PublicIncludePaths.Add(Path.Combine(ColosseumLibPath, "include"));
        PublicIncludePaths.Add(Path.Combine(ColosseumLibPath, "deps", "eigen3"));
    ...
    private bool LoadColosseumDependency(ReadOnlyTargetRules Target, string LibName, string LibFileName)
    {
        string LibrariesPath = Path.Combine(ColosseumLibPath, "deps", LibName, "lib");
    ...
    private bool AddLibDependency(string LibName, string LibPath, string LibFileName, ReadOnlyTargetRules Target, bool IsAddLibInclude)
    {
        ...
        {
            // Include path
            PublicIncludePaths.Add(Path.Combine(ColosseumLibPath, "deps", LibName, "include"));
        }
