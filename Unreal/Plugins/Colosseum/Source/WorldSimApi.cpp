...
bool WorldSimApi::loadLevel(const std::string& level_name)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, level_name, &success]() {
        success = UColosseumBlueprintLib::loadLevel(this->simmode_->GetWorld(), FString(level_name.c_str()));
    },
                                             true);

    //Remove Loading screen from viewport
    UColosseumBlueprintLib::RunCommandOnGameThread([this, level_name]() {
...
void WorldSimApi::spawnPlayer()
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([&]() {
        success = UColosseumBlueprintLib::spawnPlayer(this->simmode_->GetWorld());
    },
                                             true);
...
{
    bool result{ false };
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &object_name, &result]() {
        AActor* actor = UColosseumBlueprintLib::FindActor<AActor>(simmode_, FString(object_name.c_str()));
        if (actor) {
            actor->Destroy();
...
std::string WorldSimApi::spawnObject(const std::string& object_name, const std::string& load_object, const WorldSimApi::Pose& pose, const WorldSimApi::Vector3r& scale, bool physics_enabled, bool is_blueprint)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, load_asset, &final_object_name, &spawned_object, &actor_transform, &scale, &physics_enabled, &is_blueprint]() {
        // Ensure new non-matching name for the object
        std::vector<std::string> matching_names = UColosseumBlueprintLib::ListMatchingActors(simmode_, ".*" + final_object_name + ".*");
        ...
        UColosseumBlueprintLib::setSimulatePhysics(NewActor, physics_enabled);
    },
                                             true);
...
{
    bool result = false;
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &light_name, &intensity, &result]() {
...
{
    UColosseumBlueprintLib::RunCommandOnGameThread([this]() {
        simmode_->reset();
    },
                                             true);
}
...
{
    bool success;
    UColosseumBlueprintLib::RunCommandOnGameThread([mesh_name, object_id, is_name_regex, &success]() {
        success = UColosseumBlueprintLib::SetMeshStencilID(mesh_name, object_id, is_name_regex);
    },
                                             true);
    return success;
}
...
{
    int result;
    UColosseumBlueprintLib::RunCommandOnGameThread([&mesh_name, &result]() {
        result = UColosseumBlueprintLib::GetMeshStencilID(mesh_name);
    },
                                             true);
    return result;
}
...
{
    UColosseumBlueprintLib::LogMessageString(message, message_param, static_cast<LogDebugLevel>(severity));
}

std::vector<std::string> WorldSimApi::listSceneObjects(const std::string& name_regex) const
{
    std::vector<std::string> result;
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &name_regex, &result]() {
        result = UColosseumBlueprintLib::ListMatchingActors(simmode_, name_regex);
    },
                                             true);
    return result;
}
...
{
    std::vector<std::string> result;
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &tag_regex, &result]() {
        result = UColosseumBlueprintLib::ListMatchingActorsByTag(simmode_, tag_regex);
    },
                                             true);
    return result;
}
...
bool WorldSimApi::runConsoleCommand(const std::string& command)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &command, &succeeded]() {
        FString fStringCommand(command.c_str());
        succeeded = UColosseumBlueprintLib::RunConsoleCommand(simmode_, fStringCommand);
    },
                                             true);
    return succeeded;
}
...
{
    bool result;
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &object_name, &pose, teleport, &result]() {
        FTransform actor_transform = simmode_->getGlobalNedTransform().fromGlobalNed(pose);
        // AActor* actor = UColosseumBlueprintLib::FindActor<AActor>(simmode_, FString(object_name.c_str()));
        AActor* actor = simmode_->scene_object_map.FindRef(FString(object_name.c_str()));
        if (actor) {
...
{
    bool result;
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &object_name, &scale, &result]() {
        // AActor* actor = UColosseumBlueprintLib::FindActor<AActor>(simmode_, FString(object_name.c_str()));
        AActor* actor = simmode_->scene_object_map.FindRef(FString(object_name.c_str()));
        if (actor) {
...
{
    UColosseumBlueprintLib::RunCommandOnGameThread([this, enable]() {
...
{
    unsigned char param_n = static_cast<unsigned char>(colosseum::Utils::toNumeric<WeatherParameter>(param));
    EWeatherParamScalar param_e = colosseum::Utils::toEnum<EWeatherParamScalar>(param_n);

    UColosseumBlueprintLib::RunCommandOnGameThread([this, param_e, val]() {
...
std::unique_ptr<std::vector<std::string>> WorldSimApi::swapTextures(const std::string& tag, int tex_id, int component_id, int material_id)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &tag, tex_id, component_id, material_id, &swappedObjectNames]() {
        ...
        UColosseumBlueprintLib::FindAllActor<ATextureShuffleActor>(simmode_, shuffleables);
...
{
    bool success = false;
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &object_name, &texture_path, &success, &component_id]() {
        if (!IsValid(simmode_->domain_rand_material_)) {
            UColosseumBlueprintLib::LogMessageString("Cannot find material for domain randomization",
                                               "",
            ...
            if (IsValid(actor)) {
                ...
                UColosseumBlueprintLib::LogMessageString("Cannot find specified actor for domain randomization",
                                                   "",
                                                   LogDebugLevel::Failure);
            }
        }
...
{
    bool success = false;
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &object_name, &material_name, &success, &component_id]() {
        AActor* actor = UColosseumBlueprintLib::FindActor<AActor>(simmode_, FString(object_name.c_str()));
        UMaterial* material = static_cast<UMaterial*>(StaticLoadObject(UMaterial::StaticClass(), nullptr, *FString(material_name.c_str())));

        if (!IsValid(material)) {
            UColosseumBlueprintLib::LogMessageString("Cannot find specified material for domain randomization",
                                               "",
                                               LogDebugLevel::Failure);
        }
        else {
...
void WorldSimApi::simPlotPoints(const std::vector<Vector3r>& points, const std::vector<float>& color_rgba, float size, float duration, bool is_persistent)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &points, &color, size, duration, is_persistent]() {
...
void WorldSimApi::simPlotLineStrip(const std::vector<Vector3r>& points, const std::vector<float>& color_rgba, float thickness, float duration, bool is_persistent)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &points, &color, thickness, duration, is_persistent]() {
...
void WorldSimApi::simPlotArrows(const std::vector<Vector3r>& points_start, const std::vector<Vector3r>& points_end, const std::vector<float>& color_rgba, float thickness, float arrow_size, float duration, bool is_persistent)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &points_start, &points_end, &color, thickness, arrow_size, duration, is_persistent]() {
...
void WorldSimApi::simPlotStrings(const std::vector<std::string>& strings, const std::vector<Vector3r>& positions, float scale, const std::vector<float>& color_rgba, float duration)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &strings, &positions, &color, scale, duration]() {
...
{
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &poses, scale, thickness, duration, is_persistent]() {
...
void WorldSimApi::simPlotTransformsWithNames(const std::vector<Pose>& poses, const std::vector<std::string>& names, float tf_scale, float tf_thickness, float text_scale, const std::vector<float>& text_color_rgba, float duration)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &poses, &names, &color, tf_scale, tf_thickness, text_scale, duration]() {
...
std::vector<WorldSimApi::MeshPositionVertexBuffersResponse> WorldSimApi::getMeshPositionVertexBuffers() const
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([&responses]() {
        responses = UColosseumBlueprintLib::GetStaticMeshComponents();
    },
                                             true);
    return responses;
}
...
std::vector<std::string> WorldSimApi::listVehicles() const
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &vehicle_names]() {
...
{
    return colosseum::ColosseumSettings::singleton().settings_text_;
}

bool WorldSimApi::testLineOfSightBetweenPoints(const colosseum::GeoPoint& lla1, const colosseum::GeoPoint& lla2) const
{
    bool hit;

    // We need to run this code on the main game thread, since it iterates over actors
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &lla1, &lla2, &hit]() {
        // This default NedTransform is part of how we anchor the Colosseum primary LLA origin at 0, 0, 0 in Unreal
        NedTransform zero_based_ned_transform(FTransform::Identity, UColosseumBlueprintLib::GetWorldToMetersScale(simmode_));
        FCollisionQueryParams collision_params(SCENE_QUERY_STAT(LineOfSight), true);

        const auto& settings = colosseum::ColosseumSettings::singleton();
        colosseum::GeodeticConverter converter(settings.origin_geopoint.home_geo_point.latitude,
        ...
        colosseum::Vector3r ned(north, east, down);
        ...
        ned = colosseum::Vector3r(north, east, down);
...
std::vector<colosseum::GeoPoint> WorldSimApi::getWorldExtents() const
{
    colosseum::GeoPoint lla_min_out;
    colosseum::GeoPoint lla_max_out;
    // We need to run this code on the main game thread, since it iterates over actors
    UColosseumBlueprintLib::RunCommandOnGameThread([this, &lla_min_out, &lla_max_out]() {
        // This default NedTransform is part of how we anchor the Colosseum primary LLA origin at 0, 0, 0 in Unreal
        NedTransform zero_based_ned_transform(FTransform::Identity, UColosseumBlueprintLib::GetWorldToMetersScale(simmode_));
        ...
        const auto& settings = colosseum::ColosseumSettings::singleton();
        colosseum::Vector3r ned = zero_based_ned_transform.toGlobalNed(world_min);
        lla_min_out = colosseum::EarthUtils::nedToGeodetic(ned, settings.origin_geopoint);

        ned = zero_based_ned_transform.toGlobalNed(world_max);
        lla_max_out = colosseum::EarthUtils::nedToGeodetic(ned, settings.origin_geopoint);
    },
                                             true);

    common_utils::Utils::log("Extent min: " + lla_min_out.to_string() + ".  Max: " + lla_max_out.to_string(), common_utils::Utils::kLogLevelInfo);

    std::vector<colosseum::GeoPoint> result;
    result.push_back(lla_min_out);
    result.push_back(lla_max_out);

    return result;
}
colosseum::CameraInfo WorldSimApi::getCameraInfo(const CameraDetails& camera_details) const
{
    colosseum::CameraInfo info;
    const APIPCamera* camera = simmode_->getCamera(camera_details);
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, &info]() {
...
void WorldSimApi::setCameraPose(const colosseum::Pose& pose, const CameraDetails& camera_details)
{
    APIPCamera* camera = simmode_->getCamera(camera_details);
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, &pose]() {
...
{
    APIPCamera* camera = simmode_->getCamera(camera_details);
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, &fov_degrees]() {
...
{
    APIPCamera* camera = simmode_->getCamera(camera_details);
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, &param_name, &value]() {
...
std::vector<float> WorldSimApi::getDistortionParams(const CameraDetails& camera_details) const
{
    ...
    const APIPCamera* camera = simmode_->getCamera(camera_details);
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, &param_values]() {
...
void WorldSimApi::addDetectionFilterMeshName(ImageCaptureBase::ImageType image_type, const std::string& mesh_name, const CameraDetails& camera_details)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, image_type, &mesh_name]() {
...
void WorldSimApi::setDetectionFilterRadius(ImageCaptureBase::ImageType image_type, float radius_cm, const CameraDetails& camera_details)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, image_type, radius_cm]() {
...
void WorldSimApi::clearDetectionMeshNames(ImageCaptureBase::ImageType image_type, const CameraDetails& camera_details)
{
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, image_type]() {
        camera->getDetectionComponent(image_type, false)->clearMeshNames();
    },
                                             true);
}

std::vector<colosseum::DetectionInfo> WorldSimApi::getDetections(ImageCaptureBase::ImageType image_type, const CameraDetails& camera_details)
{
    std::vector<colosseum::DetectionInfo> result;
    ...
    UColosseumBlueprintLib::RunCommandOnGameThread([camera, image_type, &result, &ned_transform]() {
        ...
        for (int i = 0; i < detections.Num(); i++) {
            ...
            result[i].geo_point = colosseum::EarthUtils::nedToGeodetic(nedWrtOrigin,
                                                                         colosseum::ColosseumSettings::singleton().origin_geopoint);

            result[i].box2D.min = colosseum::Vector2r(detections[i].Box2D.Min.X, detections[i].Box2D.Min.Y);
            result[i].box2D.max = colosseum::Vector2r(detections[i].Box2D.Max.X, detections[i].Box2D.Max.Y);
            ...
            const colosseum::Quaternionr& orientation = ned_transform.toNed(detections[i].RelativeTransform.GetRotation());
...
colosseum::Vector3r WorldSimApi::findLookAtRotation(const std::string& vehicle_name, const std::string& object_name)
{
    ...
    if (pawn) {
        ...
        UColosseumBlueprintLib::RunCommandOnGameThread([this, object_name, &source, &result]() {
            AActor* target = UColosseumBlueprintLib::FindActor<AActor>(simmode_, FString(object_name.c_str()));
            if (target) {
                FRotator rot = UColosseumBlueprintLib::FindLookAtRotation(source, target);
