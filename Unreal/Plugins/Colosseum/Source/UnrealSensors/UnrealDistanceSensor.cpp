...
UnrealDistanceSensor::UnrealDistanceSensor(const ColosseumSettings::DistanceSetting& setting,
                                           AActor* actor, const NedTransform* ned_transform)
    : DistanceSimple(setting), actor_(actor), ned_transform_(ned_transform)
{
}

colosseum::real_T UnrealDistanceSensor::getRayLength(const colosseum::Pose& pose)
{
    //update ray tracing
    ...
    bool is_hit = UColosseumBlueprintLib::GetObstacle(actor_, ned_transform_->fromLocalNed(start), ned_transform_->fromLocalNed(end), dist_hit);
    ...
    //UColosseumBlueprintLib::LogMessage(FString("Distance to "), hit_name+FString(": ")+FString::SanitizeFloat(distance), LogDebugLevel::Informational);
