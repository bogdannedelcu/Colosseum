...
// ctor
UnrealLidarSensor::UnrealLidarSensor(const ColosseumSettings::LidarSetting& setting,
...
{
    const colosseum::LidarSimpleParams params = getParams();
...
void UnrealLidarSensor::getPointCloud(const colosseum::Pose& lidar_pose, const colosseum::Pose& vehicle_pose,
                                      const colosseum::TTimeDelta delta_time, colosseum::vector<colosseum::real_T>& point_cloud, colosseum::vector<int>& segmentation_cloud)
{
    point_cloud.clear();
    ...
    if (total_points_to_scan > MAX_POINTS_IN_SCAN) {
        ...
        UColosseumBlueprintLib::LogMessageString("Lidar: ", "Capping number of points to scan", LogDebugLevel::Failure);
    }
    ...
    if (points_to_scan_with_one_laser <= 0) {
        //UColosseumBlueprintLib::LogMessageString("Lidar: ", "No points requested this frame", LogDebugLevel::Failure);
        return;
    }
...
bool UnrealLidarSensor::shootLaser(const colosseum::Pose& lidar_pose, const colosseum::Pose& vehicle_pose,
                                   ...
                                   const colosseum::LidarSimpleParams& params, Vector3r& point, int& segmentationID)
{
    ...
    colosseum::Quaternionr ray_q_l = colosseum::VectorMath::toQuaternion(
        colosseum::Utils::degreesToRadians(vertical_angle), //pitch - rotation around Y axis
        0, //roll  - rotation around X axis
        colosseum::Utils::degreesToRadians(horizontal_angle)); //yaw   - rotation around Z axis

    // get ray quaternion in body frame
    colosseum::Quaternionr ray_q_b = VectorMath::coordOrientationAdd(ray_q_l, lidar_pose.orientation);

    // get ray quaternion in world frame
    colosseum::Quaternionr ray_q_w = VectorMath::coordOrientationAdd(ray_q_b, vehicle_pose.orientation);
    ...
    bool is_hit = UColosseumBlueprintLib::GetObstacle(actor_, ned_transform_->fromLocalNed(start), ned_transform_->fromLocalNed(end), hit_result, actor_, ECC_Visibility);
    ...
    if (is_hit) {
        ...
        if (false && UColosseumBlueprintLib::IsInGameThread()) {
        ...
        case ColosseumSettings::LidarSetting::DataFrame::VehicleInertialFrame:
            ...
            break;
        case ColosseumSettings::LidarSetting::DataFrame::SensorLocalFrame:
