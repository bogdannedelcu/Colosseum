...
void ASimModeWorldMultiRotor::setupClockSpeed()
{
    ...
    if (clock_type == "ScalableClock") {
        ...
        //static_cast<colosseum::TTimeDelta>(getPhysicsLoopPeriod() * 1E-9 * clock_speed)));

        //Approach 2: scale control loop frequency if clock is speeded up
        if (clock_speed >= 1) {
            ClockFactory::get(std::make_shared<colosseum::SteppableClock>(
                static_cast<colosseum::TTimeDelta>(getPhysicsLoopPeriod() * 1E-9))); //no clock_speed multiplier
...
std::unique_ptr<colosseum::ApiServerBase> ASimModeWorldMultiRotor::createApiServer() const
{
#ifdef COLOSSEUM_NO_RPC
    return ASimModeBase::createApiServer();
#else
    return std::unique_ptr<colosseum::ApiServerBase>(new colosseum::MultirotorRpcLibServer(
...
{
    return ((vehicle_type == ColosseumSettings::kVehicleTypeSimpleFlight) ||
            (vehicle_type == ColosseumSettings::kVehicleTypePX4) ||
            (vehicle_type == ColosseumSettings::kVehicleTypeArduCopterSolo) ||
            (vehicle_type == ColosseumSettings::kVehicleTypeArduCopter));
}

std::string ASimModeWorldMultiRotor::getVehiclePawnPathName(const ColosseumSettings::VehicleSetting& vehicle_setting) const
{
...
colosseum::VehicleApiBase* ASimModeWorldMultiRotor::getVehicleApi(const PawnSimApi::Params& pawn_sim_api_params,
