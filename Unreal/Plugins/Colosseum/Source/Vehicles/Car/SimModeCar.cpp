...
void ASimModeCar::setupClockSpeed()
{
    ...
    UColosseumBlueprintLib::setUnrealClockSpeed(this, current_clockspeed_);
    UColosseumBlueprintLib::LogMessageString("Clock Speed: ", std::to_string(current_clockspeed_), LogDebugLevel::Informational);
}
...
std::unique_ptr<colosseum::ApiServerBase> ASimModeCar::createApiServer() const
{
#ifdef COLOSSEUM_NO_RPC
    return ASimModeBase::createApiServer();
#else
    return std::unique_ptr<colosseum::ApiServerBase>(new colosseum::CarRpcLibServer(
...
{
    return ((vehicle_type == ColosseumSettings::kVehicleTypePhysXCar) ||
            (vehicle_type == ColosseumSettings::kVehicleTypeArduRover));
}

std::string ASimModeCar::getVehiclePawnPathName(const ColosseumSettings::VehicleSetting& vehicle_setting) const
{
...
colosseum::VehicleApiBase* ASimModeCar::getVehicleApi(const PawnSimApi::Params& pawn_sim_api_params,
