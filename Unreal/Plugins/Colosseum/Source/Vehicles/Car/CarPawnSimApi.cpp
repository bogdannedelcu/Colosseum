...
void CarPawnSimApi::initialize()
{
    ...
    joystick_controls_ = colosseum::CarApiBase::CarControls();
}
...
void CarPawnSimApi::updateRendering(float dt)
{
    ...
    for (auto i = 0; i < vehicle_api_messages_.size(); ++i) {
        UColosseumBlueprintLib::LogMessage(FString(vehicle_api_messages_[i].c_str()), TEXT(""), LogDebugLevel::Success, 30);
    }
    ...
    try {
        ...
        UColosseumBlueprintLib::LogMessage(FString(e.what()), TEXT(""), LogDebugLevel::Failure, 30);
    }
}
...
void CarPawnSimApi::updateCarControls()
{
    ...
    if (rc_data.is_initialized) {
        if (!rc_data.is_valid) {
            UColosseumBlueprintLib::LogMessageString("Control Mode: ", "[INVALID] Wheel/Joystick", LogDebugLevel::Informational);
            return;
        }
        UColosseumBlueprintLib::LogMessageString("Control Mode: ", "Wheel/Joystick", LogDebugLevel::Informational);
        ...
        UColosseumBlueprintLib::LogMessageString("Control Mode: ", "Keyboard", LogDebugLevel::Informational);
    ...
    if (!vehicle_api_->isApiControlEnabled()) {
        ...
        UColosseumBlueprintLib::LogMessageString("Control Mode: ", "API", LogDebugLevel::Informational);
        current_controls_ = vehicle_api_->getCarControls();
        pawn_api_->updateMovement(current_controls_);
    }
    UColosseumBlueprintLib::LogMessageString("Accel: ", std::to_string(current_controls_.throttle), LogDebugLevel::Informational);
    UColosseumBlueprintLib::LogMessageString("Break: ", std::to_string(current_controls_.brake), LogDebugLevel::Informational);
    UColosseumBlueprintLib::LogMessageString("Steering: ", std::to_string(current_controls_.steering), LogDebugLevel::Informational);
    UColosseumBlueprintLib::LogMessageString("Handbrake: ", std::to_string(current_controls_.handbrake), LogDebugLevel::Informational);
    UColosseumBlueprintLib::LogMessageString("Target Gear: ", std::to_string(current_controls_.manual_gear), LogDebugLevel::Informational);
}
