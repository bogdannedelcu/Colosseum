...
ACarPawn::ACarPawn()
{
    static ConstructorHelpers::FClassFinder<APIPCamera> pip_camera_class(TEXT("Blueprint'/Colosseum/Blueprints/BP_PIPCamera'"));
    pip_camera_class_ = pip_camera_class.Succeeded() ? pip_camera_class.Class : nullptr;

    const auto& car_mesh_paths = ColosseumSettings::singleton().pawn_paths["DefaultCar"];
    auto slippery_mat = Cast<UPhysicalMaterial>(
        UColosseumBlueprintLib::LoadObject(car_mesh_paths.slippery_mat));
    auto non_slippery_mat = Cast<UPhysicalMaterial>(
        UColosseumBlueprintLib::LoadObject(car_mesh_paths.non_slippery_mat));
    if (slippery_mat)
        slippery_mat_ = slippery_mat;
    else
        UColosseumBlueprintLib::LogMessageString("Failed to load Slippery physics material", "", LogDebugLevel::Failure);
    if (non_slippery_mat)
        non_slippery_mat_ = non_slippery_mat;
    else
        UColosseumBlueprintLib::LogMessageString("Failed to load NonSlippery physics material", "", LogDebugLevel::Failure);
    ...
    ConstructorHelpers::FObjectFinder<USoundCue> SoundCue(TEXT("/Colosseum/VehicleAdv/Sound/Engine_Loop_Cue.Engine_Loop_Cue"));
...
void ACarPawn::updateHUDStrings()
{
    ...
    float speed_unit_factor = ColosseumSettings::singleton().speed_unit_factor;
    FText speed_unit_label = FText::FromString(FString(ColosseumSettings::singleton().speed_unit_label.c_str()));
    ...
    UChaosWheeledVehicleMovementComponent* movement = CastChecked<UChaosWheeledVehicleMovementComponent>(getVehicleMovementComponent());
    UColosseumBlueprintLib::LogMessage(TEXT("Speed: "), last_speed_.ToString(), LogDebugLevel::Informational);
    UColosseumBlueprintLib::LogMessage(TEXT("Gear: "), last_gear_.ToString(), LogDebugLevel::Informational);
    UColosseumBlueprintLib::LogMessage(TEXT("RPM: "), FText::AsNumber(movement->GetEngineRotationSpeed()).ToString(), LogDebugLevel::Informational);
}
...
void ACarPawn::setupInputBindings()
{
    ...
    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("MoveForward", EKeys::Up, 1), this, this, &ACarPawn::onMoveForward);

    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("MoveForward", EKeys::Down, -1), this, this, &ACarPawn::onMoveForward);

    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("MoveRight", EKeys::Right, 0.5), this, this, &ACarPawn::onMoveRight);

    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("MoveRight", EKeys::Left, -0.5), this, this, &ACarPawn::onMoveRight);

    UColosseumBlueprintLib::BindActionToKey("Handbrake", EKeys::End, this, &ACarPawn::onHandbrakePressed, true);
    UColosseumBlueprintLib::BindActionToKey("Handbrake", EKeys::End, this, &ACarPawn::onHandbrakeReleased, false);

    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("Footbrake", EKeys::SpaceBar, 1), this, this, &ACarPawn::onFootBrake);

    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("MoveRight", EKeys::Gamepad_LeftX, 1), this, this, &ACarPawn::onMoveRight);

    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("MoveForward", EKeys::Gamepad_RightTriggerAxis, 1), this, this, &ACarPawn::onMoveForward);

    UColosseumBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("Footbrake", EKeys::Gamepad_LeftTriggerAxis, 1), this, this, &ACarPawn::onFootBrake);

    //below is not needed
    //UColosseumBlueprintLib::BindActionToKey("Reverse", EKeys::Down, this, &ACarPawn::onReversePressed, true);
    //UColosseumBlueprintLib::BindActionToKey("Reverse", EKeys::Down, this, &ACarPawn::onReverseReleased, false);
}
