...
UCLASS()
class COLOSSEUM_API ASimModeBase : public AActor
{
public:
    GENERATED_BODY()
    ...
    UFUNCTION(BlueprintPure, Category = "Colosseum | get stuff")
    static ASimModeBase* getSimMode();

    UFUNCTION(BlueprintCallable, Category = "Colosseum | get stuff")
    ...
    virtual void setWind(const colosseum::Vector3r& wind) const;
    virtual void setExtForce(const colosseum::Vector3r& ext_force) const;
    ...
    colosseum::ApiProvider* getApiProvider() const
    {
    ...
    const APIPCamera* getCamera(const colosseum::CameraDetails& camera_details) const;

    APIPCamera* getCamera(const colosseum::CameraDetails& camera_details)
    {
...
protected: //must overrides
    ...
    virtual std::unique_ptr<colosseum::ApiServerBase> createApiServer() const;
    ...
    virtual std::string getVehiclePawnPathName(const ColosseumSettings::VehicleSetting& vehicle_setting) const;
    ...
    virtual void registerPhysicsBody(colosseum::VehicleSimApiBase* physicsBody);

protected: //optional overrides
    virtual APawn* createVehiclePawn(const ColosseumSettings::VehicleSetting& vehicle_setting);
...
protected: //Utility methods for derived classes
    virtual const ColosseumSettings& getSettings() const;
    FRotator toFRotator(const ColosseumSettings::Rotation& rotation, const FRotator& default_val);
...
private:
    ...
    typedef colosseum::SensorBase::SensorType SensorType;
...
private:
    ...
    std::unique_ptr<colosseum::WorldSimApiBase> world_sim_api_;
    std::unique_ptr<colosseum::ApiProvider> api_provider_;
    std::unique_ptr<colosseum::ApiServerBase> api_server_;
    colosseum::StateReporterWrapper debug_reporter_;

    std::vector<std::unique_ptr<colosseum::VehicleSimApiBase>> vehicle_sim_apis_;
