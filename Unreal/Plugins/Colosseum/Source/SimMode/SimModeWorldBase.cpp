...
{
    std::vector<colosseum::UpdatableObject*> vehicles;
    ...
    physics_world_.reset(new colosseum::PhysicsWorld(std::move(physics_engine),
                                                       vehicles,
                                                       getPhysicsLoopPeriod()));
}

void ASimModeWorldBase::registerPhysicsBody(colosseum::VehicleSimApiBase* physicsBody)
{
...
std::unique_ptr<ASimModeWorldBase::PhysicsEngineBase> ASimModeWorldBase::createPhysicsEngine()
{
    ...
    if (physics_engine_name == "")
        ...
        colosseum::Settings fast_phys_settings;
        if (colosseum::Settings::singleton().getChild("FastPhysicsEngine", fast_phys_settings)) {
            physics_engine.reset(new colosseum::FastPhysicsEngine(fast_phys_settings.getBool("EnableGroundLock", true)));
        }
        else {
            physics_engine.reset(new colosseum::FastPhysicsEngine());
        }
        ...
        physics_engine.reset(new colosseum::ExternalPhysicsEngine());
    }
    else {
        physics_engine.reset();
        UColosseumBlueprintLib::LogMessageString("Unrecognized physics engine name: ", physics_engine_name, LogDebugLevel::Failure);
    }
...
void ASimModeWorldBase::setWind(const colosseum::Vector3r& wind) const
{
    physics_engine_->setWind(wind);
}

void ASimModeWorldBase::setExtForce(const colosseum::Vector3r& ext_force) const
{
    physics_engine_->setExtForce(ext_force);
}

void ASimModeWorldBase::updateDebugReport(colosseum::StateReporterWrapper& debug_reporter)
{
