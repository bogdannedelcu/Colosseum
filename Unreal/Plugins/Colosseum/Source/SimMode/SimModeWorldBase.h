...
UCLASS()
class COLOSSEUM_API ASimModeWorldBase : public ASimModeBase
{
    GENERATED_BODY()
...
public:
    ...
    virtual void setWind(const colosseum::Vector3r& wind) const override;
    virtual void setExtForce(const colosseum::Vector3r& ext_force) const override;

protected:
    void startAsyncUpdator();
    void stopAsyncUpdator();
    virtual void updateDebugReport(colosseum::StateReporterWrapper& debug_reporter) override;

    //should be called by derived class once all api_provider_ is ready to use
    void initializeForPlay();

    //used for adding physics bodies on the fly
    virtual void registerPhysicsBody(colosseum::VehicleSimApiBase* physicsBody) override;
...
private:
    ...
    typedef colosseum::PhysicsEngineBase PhysicsEngineBase;
...
private:
    std::unique_ptr<colosseum::PhysicsWorld> physics_world_;
