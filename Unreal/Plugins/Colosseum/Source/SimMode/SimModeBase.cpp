...
//it to ColosseumLib and directly implement WorldSimApiBase interface
...
ASimModeBase::ASimModeBase()
{
    ...
    static ConstructorHelpers::FClassFinder<APIPCamera> external_camera_class(TEXT("Blueprint'/Colosseum/Blueprints/BP_PIPCamera'"));
    external_camera_class_ = external_camera_class.Succeeded() ? external_camera_class.Class : nullptr;
    static ConstructorHelpers::FClassFinder<ACameraManager> camera_director_class(TEXT("Blueprint'/Colosseum/Blueprints/BP_CameraDirector'"));
    camera_director_class_ = camera_director_class.Succeeded() ? camera_director_class.Class : nullptr;

    static ConstructorHelpers::FObjectFinder<UParticleSystem> collision_display(TEXT("ParticleSystem'/Colosseum/StarterContent/Particles/P_Explosion.P_Explosion'"));
    if (!collision_display.Succeeded())
        collision_display_template = collision_display.Object;
    else
        collision_display_template = nullptr;

    static ConstructorHelpers::FClassFinder<APIPCamera> pip_camera_class_val(TEXT("Blueprint'/Colosseum/Blueprints/BP_PIPCamera'"));
    ...
    static ConstructorHelpers::FClassFinder<UUserWidget> loading_screen_class_find(TEXT("WidgetBlueprint'/Colosseum/Blueprints/BP_LoadingScreenWidget'"));
    ...
    static ConstructorHelpers::FObjectFinder<UMaterial> domain_rand_mat_finder(TEXT("Material'/Colosseum/HUDAssets/DomainRandomizationMaterial.DomainRandomizationMaterial'"));
...
void ASimModeBase::toggleLoadingScreen(bool is_visible)
{
    if (loading_screen_widget_ == nullptr)
        ...
        UColosseumBlueprintLib::RunCommandOnGameThread([this, is_visible]() {
            if (is_visible)
...
void ASimModeBase::BeginPlay()
{
    ...
    global_ned_transform_.reset(new NedTransform(player_start_transform,
                                                 UColosseumBlueprintLib::GetWorldToMetersScale(this)));

    UColosseumBlueprintLib::GenerateAssetRegistryMap(this, asset_map);

    world_sim_api_.reset(new WorldSimApi(this));
    api_provider_.reset(new colosseum::ApiProvider(world_sim_api_.get()));

    UColosseumBlueprintLib::setLogMessagesVisibility(getSettings().log_messages_visible);
    ...
    initializeTimeOfDay();
    ColosseumSettings::TimeOfDaySetting tod_setting = getSettings().tod_setting;
    ...
    UColosseumBlueprintLib::LogMessage(TEXT("Press F1 to see help"), TEXT(""), LogDebugLevel::Informational);
    ...
    UColosseumBlueprintLib::GenerateActorMap(this, scene_object_map);
    ...
    for (auto& api : api_provider_->getVehicleApis()) {
        if (api) { //sim-only vehicles may have api as null
            ...
            if (!api->isReady(message)) {
                UColosseumBlueprintLib::LogMessage("Vehicle was not initialized", "", LogDebugLevel::Failure);
                if (message.size() > 0) {
                    UColosseumBlueprintLib::LogMessage(message.c_str(), "", LogDebugLevel::Failure);
                }
                UColosseumBlueprintLib::LogMessage("Tip: check connection info in settings.json", "", LogDebugLevel::Informational);
            }
        }
    }
}
...
{
    UColosseumBlueprintLib::SetMeshNamingMethod(getSettings().segmentation_setting.mesh_naming_method);

    if (getSettings().segmentation_setting.init_method ==
        ColosseumSettings::SegmentationSetting::InitMethodType::CommonObjectsRandomIDs) {
        UColosseumBlueprintLib::InitializeMeshStencilIDs(getSettings().segmentation_setting.override_existing);
    }
    //else don't init
}
...
void ASimModeBase::initializeTimeOfDay()
{
    ...
    if (sky_spheres.Num() > 1)
        UColosseumBlueprintLib::LogMessage(TEXT("More than BP_Sky_Sphere were found. "),
                                ...
                                float celestial_clock_speed, float update_interval_secs, bool move_sun)
{
    ...
    if (is_enabled) {
        ...
        if (!sun_) {
            UColosseumBlueprintLib::LogMessage(TEXT("BP_Sky_Sphere was not found. "),
        ...
        if (sun_) {
            ...
            UColosseumBlueprintLib::LogMessageString("DateTime: ", Utils::to_string(ClockFactory::get()->nowNanos() / 1E9), LogDebugLevel::Informational);
        }
    }
...
void ASimModeBase::setWind(const colosseum::Vector3r& wind) const
{
    // should be overridden by derived class
    unused(wind);
...
void ASimModeBase::setExtForce(const colosseum::Vector3r& ext_force) const
{
    // should be overridden by derived class
    unused(ext_force);
...
std::unique_ptr<colosseum::ApiServerBase> ASimModeBase::createApiServer() const
{
...
void ASimModeBase::setupClockSpeed()
{
    ...
    if (clock_type == "ScalableClock")
        ClockFactory::get(std::make_shared<colosseum::ScalableClock>(clock_speed == 1 ? 1 : 1 / clock_speed));
    else if (clock_type == "SteppableClock")
        ClockFactory::get(std::make_shared<colosseum::SteppableClock>(
            static_cast<colosseum::TTimeDelta>(colosseum::SteppableClock::DefaultStepSize * clock_speed)));
    else
...
void ASimModeBase::showClockStats()
{
    float clock_speed = getSettings().clock_speed;
    if (clock_speed != 1) {
        UColosseumBlueprintLib::LogMessageString("ClockSpeed config, actual: ",
...
void ASimModeBase::advanceTimeOfDay()
{
    ...
    if (tod_enabled_ && sky_sphere_ && sun_ && tod_move_sun_) {
        ...
        if (secs > tod_update_interval_secs_) {
            ...
            UColosseumBlueprintLib::LogMessageString("DateTime: ", Utils::to_string(cur_time), LogDebugLevel::Informational);

            auto coord = colosseum::EarthCelestial::getSunCoordinates(cur_time, settings.origin_geopoint.home_geo_point.latitude, settings.origin_geopoint.home_geo_point.longitude);
...
{
    if (sun_ && sky_sphere_) {
        UColosseumBlueprintLib::RunCommandOnGameThread([this, rotation]() {
...
void ASimModeBase::setupInputBindings()
{
    ...
    UColosseumBlueprintLib::BindActionToKey("InputEventResetAll", EKeys::BackSpace, this, &ASimModeBase::reset);
}
...
const colosseum::ColosseumSettings& ASimModeBase::getSettings() const
{
    return ColosseumSettings::singleton();
}
...
void ASimModeBase::initializeCameraDirector(const FTransform& camera_transform, float follow_distance)
{
    ...
    UColosseumBlueprintLib::FindAllActor<ACameraManager>(this, camera_dirs);
...
const APIPCamera* ASimModeBase::getCamera(const colosseum::CameraDetails& camera_details) const
{
...
void ASimModeBase::startApiServer()
{
    if (getSettings().enable_rpc) {
        ...
        try {
            ...
            UColosseumBlueprintLib::LogMessageString("Cannot start RpcLib Server", ex.what(), LogDebugLevel::Failure);
        }
    }
    else
        UColosseumBlueprintLib::LogMessageString("API server is disabled in settings", "", LogDebugLevel::Informational);
}
...
void ASimModeBase::updateDebugReport(colosseum::StateReporterWrapper& debug_reporter)
{
    ...
    if (debug_reporter.canReport()) {
        ...
        for (auto& api : getApiProvider()->getVehicleSimApis()) {
            ...
            colosseum::StateReporter& reporter = *debug_reporter.getReporter();
...
FRotator ASimModeBase::toFRotator(const colosseum::ColosseumSettings::Rotation& rotation, const FRotator& default_val)
{
...
APawn* ASimModeBase::createVehiclePawn(const ColosseumSettings::VehicleSetting& vehicle_setting)
{
    ...
    auto vehicle_bp_class = UColosseumBlueprintLib::LoadClass(
...
std::unique_ptr<PawnSimApi> ASimModeBase::createVehicleApi(APawn* vehicle_pawn)
{
    ...
    const auto& home_geopoint = colosseum::EarthUtils::nedToGeodetic(pawn_ned_pos, getSettings().origin_geopoint);
...
bool ASimModeBase::createVehicleAtRuntime(const std::string& vehicle_name, const std::string& vehicle_type,
                                          const colosseum::Pose& pose, const std::string& pawn_path)
{
    ...
    // Retroactively adjust ColosseumSettings, so it's like we knew about this vehicle all along
    ColosseumSettings::singleton().addVehicleSetting(vehicle_name, vehicle_type_lower, pose, pawn_path);
...
std::string ASimModeBase::getVehiclePawnPathName(const ColosseumSettings::VehicleSetting& vehicle_setting) const
{
    //derived class should override this method to retrieve types of pawns they support
    return "";
}
...
colosseum::VehicleApiBase* ASimModeBase::getVehicleApi(const PawnSimApi::Params& pawn_sim_api_params,
...
void ASimModeBase::drawLidarDebugPoints()
{
    ...
    for (auto& sim_api : getApiProvider()->getVehicleSimApis()) {
        ...
        colosseum::VehicleApiBase* api = getApiProvider()->getVehicleApi(vehicle_name);
        if (api != nullptr) {
            colosseum::uint count_lidars = api->getSensors().size(SensorType::Lidar);

            for (colosseum::uint i = 0; i < count_lidars; i++) {
                // TODO: Is it incorrect to assume LidarSimple here?
                const colosseum::LidarSimple* lidar =
                    static_cast<const colosseum::LidarSimple*>(api->getSensors().getByType(SensorType::Lidar, i));
                if (lidar != nullptr && lidar->getParams().draw_debug_points) {
                    lidar_draw_debug_points_ = true;

                    colosseum::LidarData lidar_data = lidar->getOutput();
                    ...
                    for (int j = 0; j < lidar_data.point_cloud.size(); j = j + 3) {
                        ...
                        if (lidar->getParams().data_frame == ColosseumSettings::LidarSetting::DataFrame::VehicleInertialFrame) {
                            uu_point = pawn_sim_api->getNedTransform().fromLocalNed(point);
                        }
                        else if (lidar->getParams().data_frame == ColosseumSettings::LidarSetting::DataFrame::SensorLocalFrame) {
...
void ASimModeBase::drawDistanceSensorDebugPoints()
{
    ...
    for (auto& sim_api : getApiProvider()->getVehicleSimApis()) {
        ...
        if (api != nullptr) {
            colosseum::uint count_distance_sensors = api->getSensors().size(SensorType::Distance);
            Pose vehicle_pose = pawn_sim_api->getGroundTruthKinematics()->pose;

            for (colosseum::uint i = 0; i < count_distance_sensors; i++) {
                const colosseum::DistanceSimple* distance_sensor =
                    static_cast<const colosseum::DistanceSimple*>(api->getSensors().getByType(SensorType::Distance, i));

                if (distance_sensor != nullptr && distance_sensor->getParams().draw_debug_points) {
                    colosseum::DistanceSensorData distance_sensor_data = distance_sensor->getOutput();
