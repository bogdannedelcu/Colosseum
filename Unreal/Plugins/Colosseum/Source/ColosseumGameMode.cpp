...
class AUnrealLog : public colosseum::Utils::Logger
{
public:
    virtual void log(int level, const std::string& message) override
    {
        ...
        if ((tab_pos = message.find(delim)) != std::string::npos) {
            UColosseumBlueprintLib::LogMessageString(message.substr(0, tab_pos),
        ...
        if (level == colosseum::Utils::kLogLevelError) {
            UE_LOG(LogTemp, Error, TEXT("%s"), *FString(message.c_str()));
        }
        else if (level == colosseum::Utils::kLogLevelWarn) {
        ...
        colosseum::Utils::Logger::log(level, message);
    }
};

static AUnrealLog GlobalASimLog;

AColosseumGameMode::AColosseumGameMode(const FObjectInitializer& ObjectInitializer)
...
//UGameUserSettings* AColosseumGameMode::GetGameUserSettings()
//{
//    if (GEngine != nullptr)
//    {
...
void AColosseumGameMode::StartPlay()
{
    Super::StartPlay();
