...
bool UColosseumBlueprintLib::log_messages_hidden_ = false;
colosseum::ColosseumSettings::SegmentationSetting::MeshNamingMethodType UColosseumBlueprintLib::mesh_naming_method_ =
    colosseum::ColosseumSettings::SegmentationSetting::MeshNamingMethodType::OwnerName;
IImageWrapperModule* UColosseumBlueprintLib::image_wrapper_module_ = nullptr;

void UColosseumBlueprintLib::LogMessageString(const std::string& prefix, const std::string& suffix, LogDebugLevel level, float persist_sec)
{
    LogMessage(FString(prefix.c_str()), FString(suffix.c_str()), level, persist_sec);
}

EAppReturnType::Type UColosseumBlueprintLib::ShowMessage(EAppMsgType::Type message_type, const std::string& message, const std::string& title)
{
...
void UColosseumBlueprintLib::enableWorldRendering(AActor* context, bool enable)
{
...
void UColosseumBlueprintLib::setSimulatePhysics(AActor* actor, bool simulate_physics)
{
...
bool UColosseumBlueprintLib::loadLevel(UObject* context, const FString& level_name)
{
...
bool UColosseumBlueprintLib::spawnPlayer(UWorld* context)
{
...
std::vector<UPrimitiveComponent*> UColosseumBlueprintLib::getPhysicsComponents(AActor* actor)
{
...
void UColosseumBlueprintLib::resetSimulatePhysics(AActor* actor)
{
    TInlineComponentArray<UPrimitiveComponent*> components;
    actor->GetComponents(components);
...
void UColosseumBlueprintLib::enableViewportRendering(AActor* context, bool enable)
{
...
void UColosseumBlueprintLib::OnBeginPlay()
{
    image_wrapper_module_ = &FModuleManager::LoadModuleChecked<IImageWrapperModule>(FName("ImageWrapper"));
}

void UColosseumBlueprintLib::OnEndPlay()
{
    //nothing to do for now
    image_wrapper_module_ = nullptr;
}

IImageWrapperModule* UColosseumBlueprintLib::getImageWrapperModule()
{
    return image_wrapper_module_;
}

void UColosseumBlueprintLib::setLogMessagesVisibility(bool is_visible)
{
...
void UColosseumBlueprintLib::LogMessage(const FString& prefix, const FString& suffix, LogDebugLevel level, float persist_sec)
{
    ...
    case LogDebugLevel::Failure:
        ...
        //UE_LOG(LogColosseum, Error, TEXT("%s%s"), *prefix, *suffix);
        break;
...
void UColosseumBlueprintLib::GenerateAssetRegistryMap(const UObject* context, TMap<FString, FAssetData>& asset_map)
{
    UColosseumBlueprintLib::RunCommandOnGameThread([context, &asset_map]() {
        FARFilter Filter;
        ...
        // Find mesh in /Game and /Colosseum asset registry. When more plugins are added this function will have to change
...
void UColosseumBlueprintLib::GenerateActorMap(const UObject* context, TMap<FString, AActor*>& scene_object_map)
{
    auto world = context->GetWorld();
...
void UColosseumBlueprintLib::setUnrealClockSpeed(const AActor* context, float clock_speed)
{
    UColosseumBlueprintLib::RunCommandOnGameThread([context, clock_speed]() {
...
float UColosseumBlueprintLib::GetWorldToMetersScale(const AActor* context)
{
    float w2m = 100.f;
...
template <typename T>
T* UColosseumBlueprintLib::GetActorComponent(AActor* actor, FString name)
{
    TArray<T*> components;
    actor->GetComponents(components);
    T* found = nullptr;
...
template UChildActorComponent* UColosseumBlueprintLib::GetActorComponent(AActor*, FString);
template USceneCaptureComponent2D* UColosseumBlueprintLib::GetActorComponent(AActor*, FString);
template UStaticMeshComponent* UColosseumBlueprintLib::GetActorComponent(AActor*, FString);
template URotatingMovementComponent* UColosseumBlueprintLib::GetActorComponent(AActor*, FString);
template UCameraComponent* UColosseumBlueprintLib::GetActorComponent(AActor*, FString);
template UDetectionComponent* UColosseumBlueprintLib::GetActorComponent(AActor*, FString);
template UCineCameraComponent* UColosseumBlueprintLib::GetActorComponent(AActor*, FString);

bool UColosseumBlueprintLib::IsInGameThread()
{
    return ::IsInGameThread();
}

void UColosseumBlueprintLib::RunCommandOnGameThread(TFunction<void()> InFunction, bool wait, const TStatId InStatId)
{
    if (IsInGameThread())
        InFunction();
    else {
...
template <>
std::string UColosseumBlueprintLib::GetMeshName<USkinnedMeshComponent>(USkinnedMeshComponent* mesh)
{
    switch (mesh_naming_method_) {
    case colosseum::ColosseumSettings::SegmentationSetting::MeshNamingMethodType::OwnerName:
        if (mesh->GetOwner())
    ...
    case colosseum::ColosseumSettings::SegmentationSetting::MeshNamingMethodType::StaticMeshName:
...
std::string UColosseumBlueprintLib::GetMeshName(ALandscapeProxy* mesh)
{
    return std::string(TCHAR_TO_UTF8(*(mesh->GetName())));
}

void UColosseumBlueprintLib::InitializeMeshStencilIDs(bool override_existing)
{
...
bool UColosseumBlueprintLib::SetMeshStencilID(const std::string& mesh_name, int object_id,
                                        bool is_name_regex)
{
...
int UColosseumBlueprintLib::GetMeshStencilID(const std::string& mesh_name)
{
...
std::vector<std::string> UColosseumBlueprintLib::ListMatchingActors(const UObject* context, const std::string& name_regex)
{
...
std::vector<std::string> UColosseumBlueprintLib::ListMatchingActorsByTag(const UObject* context, const std::string& tag_regex)
{
    std::vector<std::string> results;
    auto world = context->GetWorld();
...
std::vector<colosseum::MeshPositionVertexBuffersResponse> UColosseumBlueprintLib::GetStaticMeshComponents()
{
    std::vector<colosseum::MeshPositionVertexBuffersResponse> meshes;
    int num_meshes = 0;
    for (TObjectIterator<UStaticMeshComponent> comp; comp; ++comp) {
        *comp;
        ...
        colosseum::MeshPositionVertexBuffersResponse mesh;
        mesh.name = name;
...
TArray<FName> UColosseumBlueprintLib::ListWorldsInRegistry()
{
    FARFilter Filter;
...
UObject* UColosseumBlueprintLib::GetMeshFromRegistry(const std::string& load_object)
{
    FARFilter Filter;
...
bool UColosseumBlueprintLib::RunConsoleCommand(const AActor* context, const FString& command)
{
...
bool UColosseumBlueprintLib::HasObstacle(const AActor* actor, const FVector& start, const FVector& end, const AActor* ignore_actor, ECollisionChannel collision_channel)
{
...
bool UColosseumBlueprintLib::GetObstacle(const AActor* actor, const FVector& start, const FVector& end,
...
bool UColosseumBlueprintLib::GetLastObstaclePosition(const AActor* actor, const FVector& start, const FVector& end,
                                               FHitResult& hit, const AActor* ignore_actor, ECollisionChannel collision_channel)
{
...
void UColosseumBlueprintLib::FollowActor(AActor* follower, const AActor* followee, const FVector& offset, bool fixed_z, float fixed_z_val)
{
    //can we see followee?
    FHitResult hit;
    if (followee == nullptr) {
...
int UColosseumBlueprintLib::RemoveAxisBinding(const FInputAxisKeyMapping& axis, FInputAxisBinding* axis_binding, AActor* actor)
{
...
float UColosseumBlueprintLib::GetDisplayGamma()
{
    return GEngine->DisplayGamma;
}

void UColosseumBlueprintLib::EnableInput(AActor* actor)
{
    actor->EnableInput(actor->GetWorld()->GetFirstPlayerController());
}

UObject* UColosseumBlueprintLib::LoadObject(const std::string& name)
{
...
UClass* UColosseumBlueprintLib::LoadClass(const std::string& name)
{
    FString str(name.c_str());
...
void UColosseumBlueprintLib::CompressImageArray(int32 width, int32 height, const TArray<FColor>& src, TArray<uint8>& dest)
{
...
bool UColosseumBlueprintLib::CompressUsingImageWrapper(const TArray<uint8>& uncompressed, const int32 width, const int32 height, TArray<uint8>& compressed)
{
    bool bSucceeded = false;
    compressed.Reset();
    if (uncompressed.Num() > 0) {
        IImageWrapperModule* ImageWrapperModule = UColosseumBlueprintLib::getImageWrapperModule();
...
void UColosseumBlueprintLib::FindAllActorByTag(const UObject* context, FName tag, TArray<AActor*>& foundActors)
{
    UGameplayStatics::GetAllActorsWithTag(context, tag, foundActors);
}

FRotator UColosseumBlueprintLib::FindLookAtRotation(AActor* source, AActor* target)
{
