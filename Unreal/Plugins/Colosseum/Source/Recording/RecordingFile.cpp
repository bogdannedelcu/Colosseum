...
void RecordingFile::appendRecord(const std::vector<colosseum::ImageCaptureBase::ImageResponse>& responses,
                                 colosseum::VehicleSimApiBase* vehicle_sim_api) const
{
    ...
    for (auto i = 0; i < responses.size(); ++i) {
        ...
        try {
            ...
            save_success = false;
            UColosseumBlueprintLib::LogMessage(TEXT("Image file save failed"), FString(ex.what()), LogDebugLevel::Failure);
        }
    }
    ...
    if (save_success || (responses.size() == 0)) {
        ...
        //UColosseumBlueprintLib::LogMessage(TEXT("Screenshot saved to:"), filePath, LogDebugLevel::Success);
    }
}
...
void RecordingFile::createFile(const std::string& file_path, const std::string& header_columns)
{
    try {
        ...
        UColosseumBlueprintLib::LogMessageString(std::string("createFile Failed for ") + file_path, ex.what(), LogDebugLevel::Failure);
    }
}
...
void RecordingFile::writeString(const std::string& str) const
{
    try {
        if (log_file_handle_) {
            ...
            UColosseumBlueprintLib::LogMessageString("Attempt to write to recording log file when file was not opened", "", LogDebugLevel::Failure);
    }
    catch (std::exception& ex) {
        UColosseumBlueprintLib::LogMessageString(std::string("file write to recording file failed "), ex.what(), LogDebugLevel::Failure);
    }
}
...
void RecordingFile::startRecording(colosseum::VehicleSimApiBase* vehicle_sim_api, const std::string& folder)
{
    try {
        ...
        if (log_filepath != "")
            ...
            UColosseumBlueprintLib::LogMessageString("Cannot start recording because path for log file is not available", "", LogDebugLevel::Failure);
            return;
        }
        ...
        if (isFileOpen()) {
            ...
            UColosseumBlueprintLib::LogMessage(TEXT("Recording: "), TEXT("Started"), LogDebugLevel::Success);
        }
        else
            UColosseumBlueprintLib::LogMessageString("Error creating log file", log_filepath.c_str(), LogDebugLevel::Failure);
    }
    catch (...) {
        UColosseumBlueprintLib::LogMessageString("Error in startRecording", "", LogDebugLevel::Failure);
    }
}
...
void RecordingFile::stopRecording(bool ignore_if_stopped)
{
    ...
    if (!isFileOpen()) {
        ...
        UColosseumBlueprintLib::LogMessage(TEXT("Recording Error"), TEXT("File was not open"), LogDebugLevel::Failure);
    }
    else
        closeFile();

    UColosseumBlueprintLib::LogMessage(TEXT("Recording: "), TEXT("Stopped"), LogDebugLevel::Success);
    UColosseumBlueprintLib::LogMessage(TEXT("Data saved to: "), FString(image_path_.c_str()), LogDebugLevel::Success);
}
