...
colosseum::WorkerThreadSignal FRecordingThread::finishing_signal_;
                                      ...
                                      const common_utils::UniqueValueMap<std::string, VehicleSimApiBase*>& vehicle_sim_apis)
{
    ...
    for (const auto& vehicle_sim_api : vehicle_sim_apis) {
        ...
        running_instance_->last_poses_[vehicle_name] = colosseum::Pose();
    }
...
bool FRecordingThread::Init()
{
    ...
    if (recording_file_) {
        UColosseumBlueprintLib::LogMessage(TEXT("Initiated recording thread"), TEXT(""), LogDebugLevel::Success);
    }
    return true;
}
...
uint32 FRecordingThread::Run()
{
    while (stop_task_counter_.GetValue() == 0) {
        ...
        if (is_ready_) {
            bool interval_elapsed = colosseum::ClockFactory::get()->elapsedSince(last_screenshot_on_) > settings_.record_interval;

            if (interval_elapsed) {
                last_screenshot_on_ = colosseum::ClockFactory::get()->nowNanos();
