...
void PawnSimApi::setStartPosition(const FVector& position, const FRotator& rotator)
{
    ...
    home_geo_point_ = colosseum::EarthUtils::nedToGeodetic(nedWrtOrigin,
                                                             ColosseumSettings::singleton().origin_geopoint);
}
...
void PawnSimApi::detectUsbRc()
{
    if (getRemoteControlID() >= 0) {
        ...
        if (rc_data_.is_initialized)
            UColosseumBlueprintLib::LogMessageString("RC Controller on USB: ", joystick_state_.pid_vid == "" ? "(Detected)" : joystick_state_.pid_vid, LogDebugLevel::Informational);
        else
            UColosseumBlueprintLib::LogMessageString("RC Controller on USB not detected: ",
...
void PawnSimApi::setupCamerasFromSettings(const common_utils::UniqueValueMap<std::string, APIPCamera*>& cameras)
{
    ...
    const auto& camera_defaults = ColosseumSettings::singleton().camera_defaults;
...
{
    //UStaticMeshComponent* bodyMesh = UColosseumBlueprintLib::GetActorComponent<UStaticMeshComponent>(this, TEXT("BodyMesh"));
                             ...
                             bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
    ...
    state_.collision_info.time_stamp = colosseum::ClockFactory::get()->nowNanos();
    ...
    UColosseumBlueprintLib::LogMessageString("Collision", Utils::stringf("#%d with %s - ObjID %d", state_.collision_info.collision_count, state_.collision_info.object_name.c_str(), state_.collision_info.object_id), LogDebugLevel::Informational);
}
...
colosseum::RCData PawnSimApi::getRCData() const
{
    joystick_.getJoyStickState(getRemoteControlID(), joystick_state_);
    ...
    if (rc_data_.is_valid) {
        ...
        UColosseumBlueprintLib::LogMessageString("Joystick (T,R,P,Y,Buttons): ", Utils::stringf("%f, %f, %f %f, %s", rc_data_.throttle, rc_data_.roll, rc_data_.pitch, rc_data_.yaw, Utils::toBinaryString(joystick_state_.buttons).c_str()), LogDebugLevel::Informational);

        //TODO: should below be at controller level info?
        UColosseumBlueprintLib::LogMessageString("RC Mode: ", rc_data_.getSwitch(0) == 0 ? "Angle" : "Rate", LogDebugLevel::Informational);
    }
...
bool PawnSimApi::testLineOfSightToPoint(const colosseum::GeoPoint& lla) const
{
    bool hit;

    // We need to run this code on the main game thread, since it iterates over actors
    UColosseumBlueprintLib::RunCommandOnGameThread([this, lla, &hit]() {
        // This default NedTransform is part of how we anchor the Colosseum primary LLA origin at 0, 0, 0 in Unreal
        NedTransform zero_based_ned_transform(FTransform::Identity, UColosseumBlueprintLib::GetWorldToMetersScale(params_.pawn));
        FCollisionQueryParams collision_params(SCENE_QUERY_STAT(LineOfSight), true, params_.pawn);

        // Transform from LLA to NED
        const auto& settings = ColosseumSettings::singleton();
        ...
        if (ColosseumSettings::singleton().show_los_debug_lines_) {
            if (hit) {
...
void PawnSimApi::reportState(colosseum::StateReporter& reporter)
{
    colosseum::VehicleSimApiBase::reportState(reporter);
...
void PawnSimApi::allowPassthroughToggleInput()
{
    ...
    UColosseumBlueprintLib::LogMessage("enable_passthrough_on_collisions: ", FString::FromInt(state_.passthrough_enabled), LogDebugLevel::Informational);
}
...
{
    UColosseumBlueprintLib::RunCommandOnGameThread([this, pose, ignore_collision]() {
...
void PawnSimApi::setDebugPose(const Pose& debug_pose)
{
    ...
    if (state_.tracing_enabled && !VectorMath::hasNan(debug_pose.position)) {
        ...
        if ((state_.last_debug_position - debug_position).SizeSquared() > 0.25) {
            ...
            UColosseumBlueprintLib::LogMessage(FString("Debug Pose: "), debug_position.ToCompactString(), LogDebugLevel::Informational);
...
void PawnSimApi::updateKinematics(float dt)
{
    ...
    next_kinematics.twist.angular = colosseum::VectorMath::toAngularVelocity(
...
const colosseum::Kinematics::State* PawnSimApi::getGroundTruthKinematics() const
{
...
const colosseum::Environment* PawnSimApi::getGroundTruthEnvironment() const
{
    return environment_.get();
}
colosseum::Kinematics* PawnSimApi::getKinematics()
{
    return kinematics_.get();
}
colosseum::Environment* PawnSimApi::getEnvironment()
{
    return environment_.get();
}
...
colosseum::VehicleApiBase* PawnSimApi::getVehicleApiBase() const
{
    return nullptr;
}
