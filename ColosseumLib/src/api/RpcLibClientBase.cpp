...
namespace colosseum
{
        ...
        typedef colosseum_rpclib::RpcLibAdaptorsBase RpcLibAdaptorsBase;
        ...
        void RpcLibClientBase::confirmConnection()
        {
            ...
            if (server_ver < server_min_ver) {
                std::cerr << std::endl
                          << ver_info << std::endl;
                std::cerr << std::endl
                          << "Colosseum server is of older version and not supported by this client. Please upgrade!" << std::endl;
            }
            else if (client_ver < client_min_ver) {
                std::cerr << std::endl
                          << ver_info << std::endl;
                std::cerr << std::endl
                          << "Colosseum client is of older version and not supported by this server. Please upgrade!" << std::endl;
            }
            else
                std::cout << std::endl
                          << ver_info << std::endl;
        ...
        colosseum::GeoPoint RpcLibClientBase::getHomeGeoPoint(const std::string& vehicle_name) const
        {
            return pimpl_->client.call("getHomeGeoPoint", vehicle_name).as<RpcLibAdaptorsBase::GeoPoint>().to();
        }

        colosseum::LidarData RpcLibClientBase::getLidarData(const std::string& lidar_name, const std::string& vehicle_name) const
        {
            return pimpl_->client.call("getLidarData", lidar_name, vehicle_name).as<RpcLibAdaptorsBase::LidarData>().to();
        }

        colosseum::ImuBase::Output RpcLibClientBase::getImuData(const std::string& imu_name, const std::string& vehicle_name) const
        {
            return pimpl_->client.call("getImuData", imu_name, vehicle_name).as<RpcLibAdaptorsBase::ImuData>().to();
        }

        colosseum::BarometerBase::Output RpcLibClientBase::getBarometerData(const std::string& barometer_name, const std::string& vehicle_name) const
        {
            return pimpl_->client.call("getBarometerData", barometer_name, vehicle_name).as<RpcLibAdaptorsBase::BarometerData>().to();
        }

        colosseum::MagnetometerBase::Output RpcLibClientBase::getMagnetometerData(const std::string& magnetometer_name, const std::string& vehicle_name) const
        {
            return pimpl_->client.call("getMagnetometerData", magnetometer_name, vehicle_name).as<RpcLibAdaptorsBase::MagnetometerData>().to();
        }

        colosseum::GpsBase::Output RpcLibClientBase::getGpsData(const std::string& gps_name, const std::string& vehicle_name) const
        {
            return pimpl_->client.call("getGpsData", gps_name, vehicle_name).as<RpcLibAdaptorsBase::GpsData>().to();
        }

        colosseum::DistanceSensorData RpcLibClientBase::getDistanceSensorData(const std::string& distance_sensor_name, const std::string& vehicle_name) const
        {
        ...
        bool RpcLibClientBase::simTestLineOfSightToPoint(const colosseum::GeoPoint& point, const std::string& vehicle_name)
        {
            return pimpl_->client.call("simTestLineOfSightToPoint", RpcLibAdaptorsBase::GeoPoint(point), vehicle_name).as<bool>();
        }

        bool RpcLibClientBase::simTestLineOfSightBetweenPoints(const colosseum::GeoPoint& point1, const colosseum::GeoPoint& point2)
        {
        ...
        vector<colosseum::GeoPoint> RpcLibClientBase::simGetWorldExtents()
        {
            ...
            vector<colosseum::GeoPoint> final_result;
        ...
        colosseum::Vector3r RpcLibClientBase::simGetObjectScale(const std::string& object_name) const
        {
            return pimpl_->client.call("simGetObjectScale", object_name).as<RpcLibAdaptorsBase::Vector3r>().to();
        }

        colosseum::Pose RpcLibClientBase::simGetObjectPose(const std::string& object_name) const
        {
            return pimpl_->client.call("simGetObjectPose", object_name).as<RpcLibAdaptorsBase::Pose>().to();
        }

        bool RpcLibClientBase::simSetObjectPose(const std::string& object_name, const colosseum::Pose& pose, bool teleport)
        {
            return pimpl_->client.call("simSetObjectPose", object_name, RpcLibAdaptorsBase::Pose(pose), teleport).as<bool>();
        }

        bool RpcLibClientBase::simSetObjectScale(const std::string& object_name, const colosseum::Vector3r& scale)
        {
        ...
        colosseum::Kinematics::State RpcLibClientBase::simGetGroundTruthKinematics(const std::string& vehicle_name) const
        {
            return pimpl_->client.call("simGetGroundTruthKinematics", vehicle_name).as<RpcLibAdaptorsBase::KinematicsState>().to();
        }
        colosseum::Environment::State RpcLibClientBase::simGetGroundTruthEnvironment(const std::string& vehicle_name) const
        {
            return pimpl_->client.call("simGetGroundTruthEnvironment", vehicle_name).as<RpcLibAdaptorsBase::EnvironmentState>().to();
            ;
        }
        bool RpcLibClientBase::simCreateVoxelGrid(const colosseum::Vector3r& position, const int& x, const int& y, const int& z, const float& res, const std::string& output_file)
        {
            return pimpl_->client.call("simCreateVoxelGrid", RpcLibAdaptorsBase::Vector3r(position), x, y, z, res, output_file).as<bool>();
        }

        colosseum::Vector3r RpcLibClientBase::simFindLookAtRotation(const std::string& vehicle_name, const std::string& object_name) const
        {
