...
{
    vel_cmd_ = colosseum_ros_pkgs::VelCmd();
    // ROS params
    ...
    // ROS publishers
    colosseum_vel_cmd_world_frame_pub_ = nh_private_.advertise<colosseum_ros_pkgs::VelCmd>("/colosseum_node/" + vehicle_name + "/vel_cmd_world_frame", 1);

    // ROS subscribers
    colosseum_odom_sub_ = nh_.subscribe("/colosseum_node/" + vehicle_name + "/odom_local_ned", 50, &PIDPositionController::colosseum_odom_cb, this);
    home_geopoint_sub_ = nh_.subscribe("/colosseum_node/home_geo_point", 50, &PIDPositionController::home_geopoint_cb, this);
    // todo publish this under global nodehandle / "colosseum node" and hide it from user
    local_position_goal_srvr_ = nh_.advertiseService("/colosseum_node/local_position_goal", &PIDPositionController::local_position_goal_srv_cb, this);
    local_position_goal_override_srvr_ = nh_.advertiseService("/colosseum_node/local_position_goal/override", &PIDPositionController::local_position_goal_srv_override_cb, this);
    gps_goal_srvr_ = nh_.advertiseService("/colosseum_node/gps_goal", &PIDPositionController::gps_goal_srv_cb, this);
    gps_goal_override_srvr_ = nh_.advertiseService("/colosseum_node/gps_goal/override", &PIDPositionController::gps_goal_srv_override_cb, this);
...
void PIDPositionController::colosseum_odom_cb(const nav_msgs::Odometry& odom_msg)
{
    has_odom_ = true;
    curr_odom_ = odom_msg;
...
bool PIDPositionController::local_position_goal_srv_cb(colosseum_ros_pkgs::SetLocalPosition::Request& request, colosseum_ros_pkgs::SetLocalPosition::Response& response)
{
...
bool PIDPositionController::local_position_goal_srv_override_cb(colosseum_ros_pkgs::SetLocalPosition::Request& request, colosseum_ros_pkgs::SetLocalPosition::Response& response)
{
    // this tells the update timer callback to not do active hovering
    if (!got_goal_once_)
        got_goal_once_ = true;
...
void PIDPositionController::home_geopoint_cb(const colosseum_ros_pkgs::GPSYaw& gps_msg)
{
    if (has_home_geo_)
        return;
...
bool PIDPositionController::gps_goal_srv_cb(colosseum_ros_pkgs::SetGPSPosition::Request& request, colosseum_ros_pkgs::SetGPSPosition::Response& response)
{
    response.success = false;
...
// todo do relative altitude, or add an option for the same?
bool PIDPositionController::gps_goal_srv_override_cb(colosseum_ros_pkgs::SetGPSPosition::Request& request, colosseum_ros_pkgs::SetGPSPosition::Response& response)
{
    response.success = false;
