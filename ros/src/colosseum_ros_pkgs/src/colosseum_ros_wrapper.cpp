...
// PLUGINLIB_EXPORT_CLASS(ColosseumROSWrapper, nodelet::Nodelet)
...
ColosseumROSWrapper::ColosseumROSWrapper(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private, const std::string& host_ip)
    ...
    , tf_listener_(tf_buffer_)
{
    ...
    std::cout << "ColosseumROSWrapper Initialized!\n";
}
...
void ColosseumROSWrapper::initialize_colosseum()
{
    ...
    try {
        ...
        origin_geo_point_ = colosseum_client_->getHomeGeoPoint("");
...
void ColosseumROSWrapper::initialize_ros()
{
    ...
    nh_private_.getParam("update_colosseum_control_every_n_sec", update_colosseum_control_every_n_sec);
    ...
    isENU_ = !(odom_frame_id_ == COLOSSEUM_ODOM_FRAME_ID);
    ...
    colosseum_control_update_timer_ = nh_private_.createTimer(ros::Duration(update_colosseum_control_every_n_sec), &ColosseumROSWrapper::drone_state_timer_cb, this);
}
...
void ColosseumROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    gimbal_angle_quat_cmd_sub_ = nh_private_.subscribe("gimbal_angle_quat_cmd", 50, &ColosseumROSWrapper::gimbal_angle_quat_cmd_cb, this);
    gimbal_angle_euler_cmd_sub_ = nh_private_.subscribe("gimbal_angle_euler_cmd", 50, &ColosseumROSWrapper::gimbal_angle_euler_cmd_cb, this);
    origin_geo_point_pub_ = nh_private_.advertise<colosseum_ros_pkgs::GPSYaw>("origin_geo_point", 10);
    ...
    for (const auto& curr_vehicle_elem : ColosseumSettings::singleton().vehicles) {
        ...
        vehicle_ros->env_pub = nh_private_.advertise<colosseum_ros_pkgs::Environment>(curr_vehicle_name + "/environment", 10);
        ...
        if (colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
            ...
            drone->vel_cmd_body_frame_sub = nh_private_.subscribe<colosseum_ros_pkgs::VelCmd>(
                curr_vehicle_name + "/vel_cmd_body_frame",
                1,
                boost::bind(&ColosseumROSWrapper::vel_cmd_body_frame_cb, this, _1, vehicle_ros->vehicle_name));
            // TODO: ros::TransportHints().tcpNoDelay();

            drone->vel_cmd_world_frame_sub = nh_private_.subscribe<colosseum_ros_pkgs::VelCmd>(
                curr_vehicle_name + "/vel_cmd_world_frame",
                1,
                boost::bind(&ColosseumROSWrapper::vel_cmd_world_frame_cb, this, _1, vehicle_ros->vehicle_name));

            drone->takeoff_srvr = nh_private_.advertiseService<colosseum_ros_pkgs::Takeoff::Request, colosseum_ros_pkgs::Takeoff::Response>(
                curr_vehicle_name + "/takeoff",
                boost::bind(&ColosseumROSWrapper::takeoff_srv_cb, this, _1, _2, vehicle_ros->vehicle_name));

            drone->land_srvr = nh_private_.advertiseService<colosseum_ros_pkgs::Land::Request, colosseum_ros_pkgs::Land::Response>(
                curr_vehicle_name + "/land",
                boost::bind(&ColosseumROSWrapper::land_srv_cb, this, _1, _2, vehicle_ros->vehicle_name));

            // vehicle_ros.reset_srvr = nh_private_.advertiseService(curr_vehicle_name + "/reset",&ColosseumROSWrapper::reset_srv_cb, this);
        }
        else {
            auto car = static_cast<CarROS*>(vehicle_ros.get());
            car->car_cmd_sub = nh_private_.subscribe<colosseum_ros_pkgs::CarControls>(
                curr_vehicle_name + "/car_cmd",
                1,
                boost::bind(&ColosseumROSWrapper::car_cmd_cb, this, _1, vehicle_ros->vehicle_name));

            car->car_state_pub = nh_private_.advertise<colosseum_ros_pkgs::CarState>(curr_vehicle_name + "/car_state", 10);
        }
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings) {
                ...
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    const ImageType curr_image_type = colosseum::Utils::toEnum<ImageType>(capture_setting.image_type);
            ...
            colosseum_img_request_vehicle_name_pair_vec_.push_back({ current_image_request_vec, curr_vehicle_name });
        }
        ...
        for (const auto& [sensor_name, sensor_setting] : vehicle_setting->sensors) {
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Barometer: {
                    ...
                    sensor_publisher.publisher = nh_private_.advertise<colosseum_ros_pkgs::Altimeter>(curr_vehicle_name + "/altimeter/" + sensor_name, 10);
                    break;
                }
    ...
    if (vehicle_name_ptr_map_.size() > 1 && colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
        takeoff_all_srvr_ = nh_private_.advertiseService("all_robots/takeoff", &ColosseumROSWrapper::takeoff_all_srv_cb, this);
        land_all_srvr_ = nh_private_.advertiseService("all_robots/land", &ColosseumROSWrapper::land_all_srv_cb, this);

        // gimbal_angle_quat_cmd_sub_ = nh_.subscribe("gimbal_angle_quat_cmd", 50, &ColosseumROSWrapper::gimbal_angle_quat_cmd_cb, this);

        vel_cmd_all_body_frame_sub_ = nh_private_.subscribe("all_robots/vel_cmd_body_frame", 1, &ColosseumROSWrapper::vel_cmd_all_body_frame_cb, this);
        vel_cmd_all_world_frame_sub_ = nh_private_.subscribe("all_robots/vel_cmd_world_frame", 1, &ColosseumROSWrapper::vel_cmd_all_world_frame_cb, this);

        vel_cmd_group_body_frame_sub_ = nh_private_.subscribe("group_of_robots/vel_cmd_body_frame", 1, &ColosseumROSWrapper::vel_cmd_group_body_frame_cb, this);
        vel_cmd_group_world_frame_sub_ = nh_private_.subscribe("group_of_robots/vel_cmd_world_frame", 1, &ColosseumROSWrapper::vel_cmd_group_world_frame_cb, this);

        takeoff_group_srvr_ = nh_private_.advertiseService("group_of_robots/takeoff", &ColosseumROSWrapper::takeoff_group_srv_cb, this);
        land_group_srvr_ = nh_private_.advertiseService("group_of_robots/land", &ColosseumROSWrapper::land_group_srv_cb, this);
    }

    // todo add per vehicle reset in ColosseumLib API
    reset_srvr_ = nh_private_.advertiseService("reset", &ColosseumROSWrapper::reset_srv_cb, this);
    ...
    if (!colosseum_img_request_vehicle_name_pair_vec_.empty()) {
        ...
        nh_private_.getParam("update_colosseum_img_response_every_n_sec", update_colosseum_img_response_every_n_sec);

        ros::TimerOptions timer_options(ros::Duration(update_colosseum_img_response_every_n_sec),
                                        boost::bind(&ColosseumROSWrapper::img_response_timer_cb, this, _1),
                                        &img_timer_cb_queue_);

        colosseum_img_response_timer_ = nh_private_.createTimer(timer_options);
    ...
    if (lidar_cnt > 0) {
        ...
        ros::TimerOptions timer_options(ros::Duration(update_lidar_every_n_sec),
                                        boost::bind(&ColosseumROSWrapper::lidar_timer_cb, this, _1),
                                        &lidar_timer_cb_queue_);

        colosseum_lidar_update_timer_ = nh_private_.createTimer(timer_options);
...
bool ColosseumROSWrapper::takeoff_srv_cb(colosseum_ros_pkgs::Takeoff::Request& request, colosseum_ros_pkgs::Takeoff::Response& response, const std::string& vehicle_name)
{
...
bool ColosseumROSWrapper::takeoff_group_srv_cb(colosseum_ros_pkgs::TakeoffGroup::Request& request, colosseum_ros_pkgs::TakeoffGroup::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool ColosseumROSWrapper::takeoff_all_srv_cb(colosseum_ros_pkgs::Takeoff::Request& request, colosseum_ros_pkgs::Takeoff::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool ColosseumROSWrapper::land_srv_cb(colosseum_ros_pkgs::Land::Request& request, colosseum_ros_pkgs::Land::Response& response, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool ColosseumROSWrapper::land_group_srv_cb(colosseum_ros_pkgs::LandGroup::Request& request, colosseum_ros_pkgs::LandGroup::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool ColosseumROSWrapper::land_all_srv_cb(colosseum_ros_pkgs::Land::Request& request, colosseum_ros_pkgs::Land::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool ColosseumROSWrapper::reset_srv_cb(colosseum_ros_pkgs::Reset::Request& request, colosseum_ros_pkgs::Reset::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
colosseum::Quaternionr ColosseumROSWrapper::get_colosseum_quat(const geometry_msgs::Quaternion& geometry_msgs_quat) const
{
...
void ColosseumROSWrapper::car_cmd_cb(const colosseum_ros_pkgs::CarControls::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
// void ColosseumROSWrapper::vel_cmd_body_frame_cb(const colosseum_ros_pkgs::VelCmd& msg, const std::string& vehicle_name)
void ColosseumROSWrapper::vel_cmd_body_frame_cb(const colosseum_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
    ...
    drone->vel_cmd.drivetrain = colosseum::DrivetrainType::MaxDegreeOfFreedom;
...
void ColosseumROSWrapper::vel_cmd_group_body_frame_cb(const colosseum_ros_pkgs::VelCmdGroup& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
// void ColosseumROSWrapper::vel_cmd_all_body_frame_cb(const colosseum_ros_pkgs::VelCmd::ConstPtr& msg)
void ColosseumROSWrapper::vel_cmd_all_body_frame_cb(const colosseum_ros_pkgs::VelCmd& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void ColosseumROSWrapper::vel_cmd_world_frame_cb(const colosseum_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void ColosseumROSWrapper::vel_cmd_group_world_frame_cb(const colosseum_ros_pkgs::VelCmdGroup& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void ColosseumROSWrapper::vel_cmd_all_world_frame_cb(const colosseum_ros_pkgs::VelCmd& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void ColosseumROSWrapper::gimbal_angle_quat_cmd_cb(const colosseum_ros_pkgs::GimbalAngleQuatCmd& gimbal_angle_quat_cmd_msg)
{
...
void ColosseumROSWrapper::gimbal_angle_euler_cmd_cb(const colosseum_ros_pkgs::GimbalAngleEulerCmd& gimbal_angle_euler_cmd_msg)
{
    try {
        tf2::Quaternion quat_control_cmd;
...
colosseum_ros_pkgs::CarState ColosseumROSWrapper::get_roscarstate_msg_from_car_state(const colosseum::CarApiBase::CarState& car_state) const
{
    colosseum_ros_pkgs::CarState state_msg;
    ...
    state_msg.header.stamp = colosseum_timestamp_to_ros(car_state.timestamp);

    return state_msg;
}

nav_msgs::Odometry ColosseumROSWrapper::get_odom_msg_from_car_state(const colosseum::CarApiBase::CarState& car_state) const
{
...
nav_msgs::Odometry ColosseumROSWrapper::get_odom_msg_from_multirotor_state(const colosseum::MultirotorState& drone_state) const
{
    nav_msgs::Odometry odom_msg;
...
sensor_msgs::PointCloud2 ColosseumROSWrapper::get_lidar_msg_from_colosseum(const colosseum::LidarData& lidar_data, const std::string& vehicle_name, const std::string& sensor_name) const
{
    ...
    if (lidar_data.point_cloud.size() > 3) {
        ...
        if (isENU_) {
            try {
                ...
                auto transformStampedENU = tf_buffer_.lookupTransform(COLOSSEUM_FRAME_ID, vehicle_name, ros::Time(0), ros::Duration(1));
...
colosseum_ros_pkgs::Environment ColosseumROSWrapper::get_environment_msg_from_colosseum(const colosseum::Environment::State& env_data) const
{
...
sensor_msgs::MagneticField ColosseumROSWrapper::get_mag_msg_from_colosseum(const colosseum::MagnetometerBase::Output& mag_data) const
{
    ...
    mag_msg.header.stamp = colosseum_timestamp_to_ros(mag_data.time_stamp);

    return mag_msg;
}

// todo covariances
sensor_msgs::NavSatFix ColosseumROSWrapper::get_gps_msg_from_colosseum(const colosseum::GpsBase::Output& gps_data) const
{
    sensor_msgs::NavSatFix gps_msg;
    gps_msg.header.stamp = colosseum_timestamp_to_ros(gps_data.time_stamp);
...
sensor_msgs::Range ColosseumROSWrapper::get_range_from_colosseum(const colosseum::DistanceSensorData& dist_data) const
{
    sensor_msgs::Range dist_msg;
    dist_msg.header.stamp = colosseum_timestamp_to_ros(dist_data.time_stamp);
...
colosseum_ros_pkgs::Altimeter ColosseumROSWrapper::get_altimeter_msg_from_colosseum(const colosseum::BarometerBase::Output& alt_data) const
{
    colosseum_ros_pkgs::Altimeter alt_msg;
    alt_msg.header.stamp = colosseum_timestamp_to_ros(alt_data.time_stamp);
...
// todo covariances
sensor_msgs::Imu ColosseumROSWrapper::get_imu_msg_from_colosseum(const colosseum::ImuBase::Output& imu_data) const
{
    sensor_msgs::Imu imu_msg;
    // imu_msg.header.frame_id = "/colosseum/odom_local_ned";// todo multiple drones
    imu_msg.header.stamp = colosseum_timestamp_to_ros(imu_data.time_stamp);
...
void ColosseumROSWrapper::publish_odom_tf(const nav_msgs::Odometry& odom_msg)
{
...
colosseum_ros_pkgs::GPSYaw ColosseumROSWrapper::get_gps_msg_from_colosseum_geo_point(const colosseum::GeoPoint& geo_point) const
{
    colosseum_ros_pkgs::GPSYaw gps_msg;
...
sensor_msgs::NavSatFix ColosseumROSWrapper::get_gps_sensor_msg_from_colosseum_geo_point(const colosseum::GeoPoint& geo_point) const
{
    sensor_msgs::NavSatFix gps_msg;
    gps_msg.latitude = geo_point.latitude;
    gps_msg.longitude = geo_point.longitude;
...
ros::Time ColosseumROSWrapper::chrono_timestamp_to_ros(const std::chrono::system_clock::time_point& stamp) const
{
...
ros::Time ColosseumROSWrapper::colosseum_timestamp_to_ros(const colosseum::TTimePoint& stamp) const
{
    // colosseum appears to use chrono::system_clock with nanosecond precision
...
colosseum::MultirotorRpcLibClient* ColosseumROSWrapper::get_multirotor_client()
{
    return static_cast<colosseum::MultirotorRpcLibClient*>(colosseum_client_.get());
}

colosseum::CarRpcLibClient* ColosseumROSWrapper::get_car_client()
{
    return static_cast<colosseum::CarRpcLibClient*>(colosseum_client_.get());
}

void ColosseumROSWrapper::drone_state_timer_cb(const ros::TimerEvent& event)
{
    try {
...
ros::Time ColosseumROSWrapper::update_state()
{
    ...
    //curr_ros_time = colosseum_timestamp_to_ros(env.clock().nowNanos());
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        auto env_data = colosseum_client_->simGetGroundTruthEnvironment(vehicle_ros->vehicle_name);

        if (colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
            auto drone = static_cast<MultiRotorROS*>(vehicle_ros.get());
            drone->curr_drone_state = get_multirotor_client()->getMultirotorState(vehicle_ros->vehicle_name);

            vehicle_time = colosseum_timestamp_to_ros(drone->curr_drone_state.timestamp);
            if (!got_sim_time) {
                curr_ros_time = vehicle_time;
                got_sim_time = true;
            }

            vehicle_ros->gps_sensor_msg = get_gps_sensor_msg_from_colosseum_geo_point(drone->curr_drone_state.gps_location);
            ...
            vehicle_time = colosseum_timestamp_to_ros(car->curr_car_state.timestamp);
            if (!got_sim_time) {
                curr_ros_time = vehicle_time;
                got_sim_time = true;
            }

            vehicle_ros->gps_sensor_msg = get_gps_sensor_msg_from_colosseum_geo_point(env_data.geo_point);
            vehicle_ros->gps_sensor_msg.header.stamp = vehicle_time;

            vehicle_ros->curr_odom = get_odom_msg_from_car_state(car->curr_car_state);

            colosseum_ros_pkgs::CarState state_msg = get_roscarstate_msg_from_car_state(car->curr_car_state);
        ...
        colosseum_ros_pkgs::Environment env_msg = get_environment_msg_from_colosseum(env_data);
...
void ColosseumROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        for (auto& sensor_publisher : vehicle_ros->sensor_pubs) {
            ...
            case SensorBase::SensorType::Barometer: {
                auto baro_data = colosseum_client_->getBarometerData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
                colosseum_ros_pkgs::Altimeter alt_msg = get_altimeter_msg_from_colosseum(baro_data);
                alt_msg.header.frame_id = vehicle_ros->vehicle_name;
                sensor_publisher.publisher.publish(alt_msg);
                break;
            }
            case SensorBase::SensorType::Imu: {
                auto imu_data = colosseum_client_->getImuData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
                sensor_msgs::Imu imu_msg = get_imu_msg_from_colosseum(imu_data);
                imu_msg.header.frame_id = vehicle_ros->vehicle_name;
                sensor_publisher.publisher.publish(imu_msg);
                break;
            }
            case SensorBase::SensorType::Distance: {
                auto distance_data = colosseum_client_->getDistanceSensorData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
                sensor_msgs::Range dist_msg = get_range_from_colosseum(distance_data);
                dist_msg.header.frame_id = vehicle_ros->vehicle_name;
                sensor_publisher.publisher.publish(dist_msg);
                break;
            }
            case SensorBase::SensorType::Gps: {
                auto gps_data = colosseum_client_->getGpsData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
                sensor_msgs::NavSatFix gps_msg = get_gps_msg_from_colosseum(gps_data);
            ...
            case SensorBase::SensorType::Magnetometer: {
                auto mag_data = colosseum_client_->getMagnetometerData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
                sensor_msgs::MagneticField mag_msg = get_mag_msg_from_colosseum(mag_data);
        ...
        if (colosseum_mode_ == COLOSSEUM_MODE::DRONE) {
            ...
            if (drone->has_vel_cmd) {
                ...
                get_multirotor_client()->moveByVelocityAsync(drone->vel_cmd.x,
                                                             ...
                                                             drone->vel_cmd.z,
                                                             vel_cmd_duration_,
                                                             colosseum::DrivetrainType::MaxDegreeOfFreedom,
...
void ColosseumROSWrapper::img_response_timer_cb(const ros::TimerEvent& event)
{
    try {
...
void ColosseumROSWrapper::lidar_timer_cb(const ros::TimerEvent& event)
{
    try {
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->lidar_pubs.empty()) {
                for (auto& lidar_publisher : vehicle_name_ptr_pair.second->lidar_pubs) {
                    ...
                    sensor_msgs::PointCloud2 lidar_msg = get_lidar_msg_from_colosseum(lidar_data, vehicle_name_ptr_pair.first, lidar_publisher.sensor_name);
...
cv::Mat ColosseumROSWrapper::manual_decode_depth(const ImageResponse& img_response) const
{
...
sensor_msgs::ImagePtr ColosseumROSWrapper::get_img_msg_from_response(const ImageResponse& img_response,
                                                                  ...
                                                                  const std::string frame_id)
{
    ...
    img_msg_ptr->header.stamp = colosseum_timestamp_to_ros(img_response.time_stamp);
...
sensor_msgs::ImagePtr ColosseumROSWrapper::get_depth_img_msg_from_response(const ImageResponse& img_response,
                                                                        const ros::Time curr_ros_time,
                                                                        const std::string frame_id)
{
    ...
    depth_img_msg->header.stamp = colosseum_timestamp_to_ros(img_response.time_stamp);
...
sensor_msgs::CameraInfo ColosseumROSWrapper::generate_cam_info(const std::string& camera_name,
...
void ColosseumROSWrapper::process_and_publish_img_response(const std::vector<ImageResponse>& img_response_vec, const int img_response_idx, const std::string& vehicle_name)
{
    ...
    for (const auto& curr_img_response : img_response_vec) {
        ...
        camera_info_msg_vec_[img_response_idx_internal].header.stamp = colosseum_timestamp_to_ros(curr_img_response.time_stamp);
...
void ColosseumROSWrapper::publish_camera_tf(const ImageResponse& img_response, const ros::Time& ros_time, const std::string& frame_id, const std::string& child_frame_id)
{
    geometry_msgs::TransformStamped cam_tf_body_msg;
    cam_tf_body_msg.header.stamp = colosseum_timestamp_to_ros(img_response.time_stamp);
    ...
    cam_tf_optical_msg.header.stamp = colosseum_timestamp_to_ros(img_response.time_stamp);
...
void ColosseumROSWrapper::read_params_from_yaml_and_fill_cam_info_msg(const std::string& file_name, sensor_msgs::CameraInfo& cam_info) const
{
